---
title: "[行业观察] Hugging Face Transformers v2.0.0 - TF 2.0/PyTorch interoperability, improved tokenizers, improved torchscript support：从热闹标题回到可验证事实"
date: 2019-09-26 10:00:00
updated: 2019-09-26 10:00:00
description: "人工智能｜Hugging Face Transformers v2.0.0 - TF 2.0/PyTorch interoperability, improved tokenizers, improved torchscript support。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2019/ai-hugging-face-transformers-hugging-face-transformers-v2-0-0-t/
categories:
  - 资讯
tags:
  - 行业资讯
...
Hugging Face Transformers 在 **2019-09-26** 留下可核对记录：**Hugging Face Transformers v2.0.0 - TF 2.0/PyTorch interoperability, improved tokenizers, improved torchscript support**。AI 圈从不停，但真正值得停下来看一眼的节点通常会改写工具、成本和想象力。
...
| 一手入口 | [阅读原文](https://github.com/huggingface/transformers/releases/tag/v2.0.0) |
| 收录月 | 2019-09 |
...
- **公开信息：** each model (not just the static computation graph) can be seamlessly moved from one framework to the other during the lifetime of the model for training/evaluation/usage (frompretrained can load weights saved from models saved in one or the other framework),
...
- **公开信息：** an example is given in the quick-tour on TF 2.0 and PyTorch in the readme in which a model is trained using keras.fit before being opened in PyTorch for quick debugging/inspection.
...
- **公开信息：** resizing input embeddings to add new tokens
...
- **公开信息：** new German model (@Timoeller)
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
- **公开信息：** new script for MultipleChoice training (SWAG, RocStories...) (@erenup)
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **我想追问的：** 失败或误解的代价由谁承担：用户、平台，还是机构？
- **可执行翻译：** 若相关，就开一个 30 分钟验证任务；若不相关，也明确写“暂不跟”。
...
- **公开信息：** better fp16 support (@ziliwang and @bryant1410)
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
- **公开信息：** fix evaluation in runlmfinetuning (@SKRohit)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
对「Hugging Face Transformers v2.0.0 - TF 2.0/PyTorch interoperability, improved tokenizers, improved torchscript support」做专业诊断时，先定位它主要改写的是结构、激励，还是约束。三者抓错，后续讨论会全程错位。

请用下面的分析骨架阅读「Hugging Face Transformers v2.0.0 - TF 2.0/PyTorch interoperability, improved tokenizers, improved torchscript support」，它比单靠情绪词汇更接近机构研报的工作方式：
