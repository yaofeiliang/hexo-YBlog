---
title: "[行业观察] go-ethereum Thessian Temple (v1.9.8)：先看规则，再看热闹"
date: 2019-11-26 10:00:00
updated: 2019-11-26 10:00:00
description: "区块链｜go-ethereum Thessian Temple (v1.9.8)。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2019/blockchain-go-ethereum-go-ethereum-thessian-temple-v1-9-8/
categories:
  - 资讯
tags:
  - 行业资讯
...
go-ethereum 在 **2019-11-26** 留下可核对记录：**go-ethereum Thessian Temple (v1.9.8)**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 一手入口 | [阅读原文](https://github.com/ethereum/go-ethereum/releases/tag/v1.9.8) |
| 收录月 | 2019-11 |
...
- **公开信息：** Switch the deprecated web socket library to github.com/gorilla/websocket (20283, 20289).
...
- **公开信息：** Deprecate the gometalinter in favor of golangci-lint (20295 + dozens of linter fixes).
...
- **公开信息：** Support aliases to manually resolve name clashes in abigen generated code (20244).
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
...
- **公开信息：** Add cliquestatus API to quickly glance the health of Clique networks (20103).
...
- **公开信息：** Initial version of a light server priority API to allow incentivized services (20070).
...
- **公开信息：** Make puppeth SSH key confirmation friendlier with bad user input (20350).
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
- **公开信息：** Support loading transaction input from files in the evm utility (20273).
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
- **公开信息：** Allow digits inside of labels for the internal assembler (20362).
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 失败或误解的代价由谁承担：用户、平台，还是机构？
- **可执行翻译：** 若相关，就开一个 30 分钟验证任务；若不相关，也明确写“暂不跟”。
...
请用下面的分析骨架阅读「go-ethereum Thessian Temple (v1.9.8)」，它比单靠情绪词汇更接近机构研报的工作方式：
