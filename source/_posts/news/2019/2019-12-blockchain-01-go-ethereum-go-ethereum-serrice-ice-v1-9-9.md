---
title: "[行业观察] go-ethereum Serrice Ice (v1.9.9)：升级、托管与退出路径"
date: 2019-12-06 10:00:00
updated: 2019-12-06 10:00:00
description: "区块链｜go-ethereum Serrice Ice (v1.9.9)。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2019/blockchain-go-ethereum-go-ethereum-serrice-ice-v1-9-9/
categories:
  - 资讯
tags:
  - 行业资讯
...
go-ethereum 在 **2019-12-06** 留下可核对记录：**go-ethereum Serrice Ice (v1.9.9)**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 一手入口 | [阅读原文](https://github.com/ethereum/go-ethereum/releases/tag/v1.9.9) |
| 收录月 | 2019-12 |
...
- **公开信息：** Simplifies and cleans up the internals of the miner package (20335, 19396).
...
- **公开信息：** Fixes a regression in restoring a deleted chain from the freezer (20403).
...
- **公开信息：** Fixes a data race in the new iterator based discovery code (20421).
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
...
- **公开信息：** Fixes a pointer reference issue in ABI big.Int decoding (20412).
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
...
- **公开信息：** Fixes the help message of certain geth subcommands (20203).
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
请用下面的分析骨架阅读「go-ethereum Serrice Ice (v1.9.9)」，它比单靠情绪词汇更接近机构研报的工作方式：
