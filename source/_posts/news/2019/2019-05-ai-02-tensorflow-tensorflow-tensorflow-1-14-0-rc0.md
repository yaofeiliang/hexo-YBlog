---
title: "[行业观察] TensorFlow TensorFlow 1.14.0-rc0：这次到底方便了谁"
date: 2019-05-24 10:00:00
updated: 2019-05-24 10:00:00
description: "人工智能｜TensorFlow TensorFlow 1.14.0-rc0。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2019/ai-tensorflow-tensorflow-tensorflow-1-14-0-rc0/
categories:
  - 资讯
tags:
  - 行业资讯
...
TensorFlow 在 **2019-05-24** 留下可核对记录：**TensorFlow TensorFlow 1.14.0-rc0**。AI 圈从不停，但真正值得停下来看一眼的节点通常会改写工具、成本和想象力。
...
| 一手入口 | [阅读原文](https://github.com/tensorflow/tensorflow/releases/tag/v1.14.0-rc0) |
| 收录月 | 2019-05 |
...
- **公开信息：** This is the first 1.x release containing the compat.v2 module. This module is required to allow libraries to publish code which works in both 1.x and 2.x. After this release, no backwards incompatible changes are allowed in the 2.0 Python API.
...
- **公开信息：** Turn on MKL-DNN contraction kernels by default. MKL-DNN dynamically dispatches the best kernel implementation based on CPU vector architecture. To disable them, build with --define=tensorflowmkldnncontractionkernel=0.
...
- **公开信息：** Wraps losses passed to the compile API (strings and v1 losses) which are not instances of v2 Loss class in LossWrapper class. => All losses will now use SUMOVERBATCHSIZE reduction as default.
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** Disable runeagerly and distribution strategy if there are symbolic tensors added to the model using addmetric or addloss.
...
- **公开信息：** tf.linspace(start, stop, num) now always uses "stop" as last value (for num > 1)
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
- **公开信息：** ResourceVariable and Variable no longer accepts constraint in the constructor, nor expose it as a @property.
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 失败或误解的代价由谁承担：用户、平台，还是机构？
- **可执行翻译：** 若相关，就开一个 30 分钟验证任务；若不相关，也明确写“暂不跟”。
...
- **公开信息：** The behavior of tf.gather is now correct when axis=None and batchdims<0.
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
请用下面的分析骨架阅读「TensorFlow TensorFlow 1.14.0-rc0」，它比单靠情绪词汇更接近机构研报的工作方式：
