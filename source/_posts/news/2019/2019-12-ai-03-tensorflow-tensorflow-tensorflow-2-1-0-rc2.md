---
title: "[行业观察] TensorFlow TensorFlow 2.1.0-rc2：别被演示绕晕，先看成本和边界"
date: 2019-12-23 10:00:00
updated: 2019-12-23 10:00:00
description: "人工智能｜TensorFlow TensorFlow 2.1.0-rc2。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2019/ai-tensorflow-tensorflow-tensorflow-2-1-0-rc2/
categories:
  - 资讯
tags:
  - 行业资讯
...
2019-12-23 的「TensorFlow TensorFlow 2.1.0-rc2」值得停一下。AI 圈从不缺口号，缺的是把边界讲清楚。

TensorFlow 在 **2019-12-23** 留下可核对记录：**TensorFlow TensorFlow 2.1.0-rc2**。AI 圈从不停，但真正值得停下来看一眼的节点通常会改写工具、成本和想象力。
...
| 一手入口 | [阅读原文](https://github.com/tensorflow/tensorflow/releases/tag/v2.1.0-rc2) |
| 收录月 | 2019-12 |
...
- **公开信息：** This does not change the minimum required version for building TensorFlow from source on Windows, but builds enabling EIGENSTRONGINLINE can take over 48 hours to compile without this flag. Refer to configure.py for more information about EIGENSTRONGINLINE and /d2ReducedOptimizeHugeFunctions.
...
- **公开信息：** If either of the required DLLs, msvcp140.dll (old) or msvcp1401.dll (new), are missing on your machine, import tensorflow will print a warning message.
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
...
- **公开信息：** The tensorflow pip package is built with CUDA 10.1 and cuDNN 7.6.
...
- **公开信息：** Experimental support for mixed precision is available on GPUs and Cloud TPUs. See usage guide.
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
请用下面的分析骨架阅读「TensorFlow TensorFlow 2.1.0-rc2」，它比单靠情绪词汇更接近机构研报的工作方式：
