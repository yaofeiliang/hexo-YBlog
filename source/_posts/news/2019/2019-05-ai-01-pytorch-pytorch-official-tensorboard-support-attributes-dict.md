---
title: "[行业观察] PyTorch Official TensorBoard Support, Attributes, Dicts, Lists and User-defined types in JIT / TorchScript, Improved Distributed：给非研究员的实用导读"
date: 2019-05-01 10:00:00
updated: 2019-05-01 10:00:00
description: "人工智能｜PyTorch Official TensorBoard Support, Attributes, Dicts, Lists and User-defined types in JIT / TorchScript, Improved Distributed。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2019/ai-pytorch-pytorch-official-tensorboard-support-attributes-dict/
categories:
  - 资讯
tags:
  - 行业资讯
...
2019-05-01 的「PyTorch Official TensorBoard Support, Attributes, Dicts, Lists and User-defined types in JIT / TorchScript, Improved Distributed」值得停一下。AI 圈从不缺口号，缺的是把边界讲清楚。

PyTorch 在 **2019-05-01** 留下可核对记录：**PyTorch Official TensorBoard Support, Attributes, Dicts, Lists and User-defined types in JIT / TorchScript, Improved Distributed**。AI 圈从不停，但真正值得停下来看一眼的节点通常会改写工具、成本和想象力。
...
| 一手入口 | [阅读原文](https://github.com/pytorch/pytorch/releases/tag/v1.1.0) |
| 收录月 | 2019-05 |
...
- **公开信息：** Tensor.set: the device of a Tensor can no longer be changed via Tensor.set. This would most commonly happen when setting up a Tensor with the default CUDA device and later swapping in a Storage on a different CUDA device. Instead, set up the Tensor on the correct device from the beginning. (18832).
...
- **公开信息：** Pay attention to the order change of lrscheduler.step(). (7889).
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** torch.unique: changed the default value of sorted to True. (15379).
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** [JIT] Rename isTensor api -> isCompleteTensor. 18437
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** [JIT] Remove GraphExecutor's python bindings. 19141
...
- **公开信息：** [C++]: many methods on Type no longer exist; use the functional or Tensor method equivalent. (17991).
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
- **公开信息：** [C++]: the Backend constructor of TensorOptions no longer exists. (18137).
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
- **公开信息：** [C++, Distributed]: Remove c10d ProcessGroup::getGroupRank has been removed. (19147).
- **阅读提醒：** 兼容性信号：旧接口/旧配置可能失效，应进入变更影响评估（impact assessment），而不是只做冒烟。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
我会把 PyTorch 的公开信息先放进“可验证事实 / 机制解释 / 尚未证实的外推”三箱。只有第一箱能直接进决策材料。

请用下面的分析骨架阅读「PyTorch Official TensorBoard Support, Attributes, Dicts, Lists and User-defined types in JIT / TorchScript, Improved Distributed」，它比单靠情绪词汇更接近机构研报的工作方式：
