---
title: "[行业观察] TensorFlow TensorFlow 2.1.0-rc0：给非研究员的实用导读"
date: 2019-11-27 10:00:00
updated: 2019-11-27 10:00:00
description: "人工智能｜TensorFlow TensorFlow 2.1.0-rc0。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2019/ai-tensorflow-tensorflow-tensorflow-2-1-0-rc0/
categories:
  - 资讯
tags:
  - 行业资讯
...
TensorFlow 在 **2019-11-27** 留下可核对记录：**TensorFlow TensorFlow 2.1.0-rc0**。AI 圈从不停，但真正值得停下来看一眼的节点通常会改写工具、成本和想象力。
...
| 一手入口 | [阅读原文](https://github.com/tensorflow/tensorflow/releases/tag/v2.1.0-rc0) |
| 收录月 | 2019-11 |
...
- **公开信息：** Model.fitgenerator, Model.evaluategenerator, Model.predictgenerator, Model.trainonbatch, Model.testonbatch, and Model.predictonbatch methods now respect the runeagerly property, and will correctly run using tf.function by default.
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** Model.fitgenerator, Model.evaluategenerator, and Model.predictgenerator are deprecated endpoints. They are subsumed by Model.fit, Model.evaluate, and Model.predict which now support generators and Sequences.
...
- **公开信息：** Keras .compile .fit .evaluate and .predict are allowed to be outside of the DistributionStrategy scope, as long as the model was constructed inside of a scope.
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 失败或误解的代价由谁承担：用户、平台，还是机构？
- **可执行翻译：** 若相关，就开一个 30 分钟验证任务；若不相关，也明确写“暂不跟”。
...
- **公开信息：** Keras model.loadweights now accepts skipmismatch as an argument. This was available in external Keras, and has now been copied over to tf.keras.
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
- **公开信息：** Introduced the TextVectorization layer, which takes as input raw strings and takes care of text standardization, tokenization, n-gram generation, and vocabulary indexing. See this end-to-end text classification example.
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
- **公开信息：** Experimental support for Keras .compile, .fit, .evaluate, and .predict is available for Cloud TPU Pods.
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
请用下面的分析骨架阅读「TensorFlow TensorFlow 2.1.0-rc0」，它比单靠情绪词汇更接近机构研报的工作方式：
