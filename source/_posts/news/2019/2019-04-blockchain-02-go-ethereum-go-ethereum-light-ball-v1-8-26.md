---
title: "[行业观察] go-ethereum Light Ball (v1.8.26)：先看规则，再看热闹"
date: 2019-04-10 10:00:00
updated: 2019-04-10 10:00:00
description: "区块链｜go-ethereum Light Ball (v1.8.26)。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2019/blockchain-go-ethereum-go-ethereum-light-ball-v1-8-26/
categories:
  - 资讯
tags:
  - 行业资讯
...
go-ethereum 在 **2019-04-10** 留下可核对记录：**go-ethereum Light Ball (v1.8.26)**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 一手入口 | [阅读原文](https://github.com/ethereum/go-ethereum/releases/tag/v1.8.26) |
| 收录月 | 2019-04 |
...
- **公开信息：** Geth v1.8.26 is a tiny light client DoS hotfix.
...
- **公开信息：** We've dropped support for les/1 on master (v1.9.0 unstable) and after some light servers updated, v1.8.x light clients started crashing.
...
- **公开信息：** Turned out that les/2 on the v1.8.x release family relied on some les/1 throttling data (19437).
...
- **公开信息：** If you don't use the light client, you don't need to update.
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** Light servers don't need to update either.
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
- **公开信息：** Geth binaries and mobile libraries are available on the Geth download page.
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
对「go-ethereum Light Ball (v1.8.26)」做专业诊断时，先定位它主要改写的是结构、激励，还是约束。三者抓错，后续讨论会全程错位。

请用下面的分析骨架阅读「go-ethereum Light Ball (v1.8.26)」，它比单靠情绪词汇更接近机构研报的工作方式：
