---
title: "[行业观察] go-ethereum Tasty Tankard (v1.9.3)：把链上黑话翻译成人话"
date: 2019-09-03 10:00:00
updated: 2019-09-03 10:00:00
description: "区块链｜go-ethereum Tasty Tankard (v1.9.3)。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2019/blockchain-go-ethereum-go-ethereum-tasty-tankard-v1-9-3/
categories:
  - 资讯
tags:
  - 行业资讯
...
go-ethereum Tasty Tankard (v1.9.3) 这类节点，适合当成“制度/协议变更”，而不是当成心情天气。

go-ethereum 在 **2019-09-03** 留下可核对记录：**go-ethereum Tasty Tankard (v1.9.3)**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 一手入口 | [阅读原文](https://github.com/ethereum/go-ethereum/releases/tag/v1.9.3) |
| 收录月 | 2019-09 |
...
- **公开信息：** Add eth.fillTransaction to create-but-not-sign a transaction (19915).
...
- **公开信息：** Support DNS names in enode URLs during admin.addPeer (18524).
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
...
- **公开信息：** Enable gzip compression on the HTTP RPC API endpoint (19997).
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
...
- **公开信息：** Reduce light client default peer count from 100 to 10 (19933).
...
- **公开信息：** Align CLI flags and descriptions a bit better (19956).
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
- **公开信息：** Implement EIP-152: Blake2B F compression function precompile (19972).
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 失败或误解的代价由谁承担：用户、平台，还是机构？
- **可执行翻译：** 若相关，就开一个 30 分钟验证任务；若不相关，也明确写“暂不跟”。
...
- **公开信息：** Implement EIP-2028: Transaction data gas cost reduction (19931).
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
- **公开信息：** Implement EIP-2200: Net SSTORE optimizations (19964).
...
请用下面的分析骨架阅读「go-ethereum Tasty Tankard (v1.9.3)」，它比单靠情绪词汇更接近机构研报的工作方式：
