---
title: "[行业观察] Hugging Face Transformers Bug fix update to load the pretrained `TransfoXLModel` from s3, added fallback for OpenAIGPTTokenizer when SpaCy is not installed：别被演示绕晕，先看成本和边界"
date: 2019-02-13 10:00:00
updated: 2019-02-13 10:00:00
description: "人工智能｜Hugging Face Transformers Bug fix update to load the pretrained `TransfoXLModel` from s3, added fallback for OpenAIGPTTokenizer when SpaCy is not installed。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2019/ai-hugging-face-transformers-hugging-face-transformers-bug-fix-/
categories:
  - 资讯
tags:
  - 行业资讯
...
ai_generated: false
source_count: 1
historical_period: 2019-02
source_url: https://github.com/huggingface/transformers/releases/tag/v0.5.1
source_published: 2019-02-13
...
Hugging Face Transformers 在 **2019-02-13** 留下可核对记录：**Hugging Face Transformers Bug fix update to load the pretrained `TransfoXLModel` from s3, added fallback for OpenAIGPTTokenizer when SpaCy is not installed**。AI 圈从不停，但真正值得停下来看一眼的节点通常会改写工具、成本和想象力。
...
| 事件 | Hugging Face Transformers Bug fix update to load the pretrained `TransfoXLModel` from s3, added fallback for OpenAIGPTTokenizer when SpaCy is not installed |
| 一手入口 | [阅读原文](https://github.com/huggingface/transformers/releases/tag/v0.5.1) |
| 收录月 | 2019-02 |
...
- **公开信息：** Fixes a bug in the loading of the pretrained TransfoXLModel from the s3 dump (which is a converted TransfoXLLMHeadModel) in which the weights were not loaded.
...
- **公开信息：** Added a fallback of OpenAIGPTTokenizer on BERT's BasicTokenizer when SpaCy and ftfy are not installed. Using BERT's BasicTokenizer instead of SpaCy should be fine in most cases as long as you have a relatively clean input (SpaCy+ftfy were included to exactly reproduce the paper's pre-processing steps on the Toronto Book Corpus) and this also let us use the neversplit option to avoid splitting special tokens like [CLS], [SEP]... which is easier than adding the tokens after tokenization.
...
- **公开信息：** Updated the README on the tokenizers options and methods which was lagging behind a bit.
- **阅读提醒：** 文档信号：往往意味着官方在澄清契约边界；对接入方，契约比营销句更重要。
...
请用下面的分析骨架阅读「Hugging Face Transformers Bug fix update to load the pretrained `TransfoXLModel` from s3, added fallback for OpenAIGPTTokenizer when SpaCy is not installed」，它比单靠情绪词汇更接近机构研报的工作方式：
...
- 原始日期：2019-02-13
- 收录月份：2019-02
- 一手链接：<https://github.com/huggingface/transformers/releases/tag/v0.5.1>

## 来源与声明

- 本文是对官方公开信息的后期整理，不代表姚飞亮在 2019-02 已在本站发表本文。
