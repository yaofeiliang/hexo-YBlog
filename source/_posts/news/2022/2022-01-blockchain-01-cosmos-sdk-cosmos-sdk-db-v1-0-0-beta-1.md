---
title: "[行业观察] Cosmos SDK DB v1.0.0-beta.1：先看规则，再看热闹"
date: 2022-01-12 10:00:00
updated: 2022-01-12 10:00:00
description: "区块链｜Cosmos SDK DB v1.0.0-beta.1。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2022/blockchain-cosmos-sdk-cosmos-sdk-db-v1-0-0-beta-1/
categories:
  - 资讯
tags:
  - 行业资讯
...
Cosmos SDK 在 **2022-01-12** 留下可核对记录：**Cosmos SDK DB v1.0.0-beta.1**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 一手入口 | [阅读原文](https://github.com/cosmos/cosmos-sdk/releases/tag/db/v1.0.0-beta.1) |
| 收录月 | 2022-01 |
...
- **公开信息：** github.com/cosmos/cosmos-sdk/db (cosmos-sdk/db here an after) is a module in the Cosmos SDK repository.
...
- **公开信息：** It is used in store/v2 instead of tendermint/tm-db which is now deprecated.
...
- **公开信息：** cosmos-sdk/db extends the tendermint/tm-db interface.
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** however, it only supports BaggerDB and RocksDB (the databases we tested, which provide snapshot functionality necessary for store/v2 and ADR-040 implementation).
...
对「Cosmos SDK DB v1.0.0-beta.1」做专业诊断时，先定位它主要改写的是结构、激励，还是约束。三者抓错，后续讨论会全程错位。

请用下面的分析骨架阅读「Cosmos SDK DB v1.0.0-beta.1」，它比单靠情绪词汇更接近机构研报的工作方式：
