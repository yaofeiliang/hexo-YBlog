---
title: "[行业观察] go-ethereum Osun (v1.10.16)：把链上黑话翻译成人话"
date: 2022-02-16 10:00:00
updated: 2022-02-16 10:00:00
description: "区块链｜go-ethereum Osun (v1.10.16)。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2022/blockchain-go-ethereum-go-ethereum-osun-v1-10-16/
categories:
  - 资讯
tags:
  - 行业资讯
...
go-ethereum 在 **2022-02-16** 留下可核对记录：**go-ethereum Osun (v1.10.16)**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 一手入口 | [阅读原文](https://github.com/ethereum/go-ethereum/releases/tag/v1.10.16) |
| 收录月 | 2022-02 |
...
- **公开信息：** Block tracing via debug.traceBlockByHash has sometimes produced inconsistent/corrupt results. Fixed via (24286).
...
- **公开信息：** The --whitelist CLI parameter functionality was broken in v1.10.14, and is fixed in this release (24210).
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
...
- **公开信息：** A bug was introduced, and subsequently fixed, which could cause data corruption during mining (24349).
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
...
- **公开信息：** When signing complex datatypes in EIP712-type data, the signing-hash was incorrect. Fixed via (24220).
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
...
- **公开信息：** Evm execution times exported via metrics, were sometimes incorrect. Fixed in (24304).
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
- **公开信息：** Range prover edgecases found and fixed (24266, 24257).
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 失败或误解的代价由谁承担：用户、平台，还是机构？
- **可执行翻译：** 若相关，就开一个 30 分钟验证任务；若不相关，也明确写“暂不跟”。
...
- **公开信息：** Fix an error related to HTTP2 handling (24292).
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
- **公开信息：** A lot of spleling-mistkaes and issues related to correctness were fixed (24194, 24196, 24198, 24205, 24207, 24244, 24270, 24271, 24224, 24372, 24323, 24289, 24263 and 24211).
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
请用下面的分析骨架阅读「go-ethereum Osun (v1.10.16)」，它比单靠情绪词汇更接近机构研报的工作方式：
