---
title: "[行业观察] Hugging Face Transformers v4.23.0: Whisper, Deformable DETR, Conditional DETR, MarkupLM, MSN, `safetensors`：这次到底方便了谁"
date: 2022-10-10 10:00:00
updated: 2022-10-10 10:00:00
description: "人工智能｜Hugging Face Transformers v4.23.0: Whisper, Deformable DETR, Conditional DETR, MarkupLM, MSN, `safetensors`。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2022/ai-hugging-face-transformers-hugging-face-transformers-v4-23-0-/
categories:
  - 资讯
tags:
  - 行业资讯
...
Hugging Face Transformers 在 **2022-10-10** 留下可核对记录：**Hugging Face Transformers v4.23.0: Whisper, Deformable DETR, Conditional DETR, MarkupLM, MSN, `safetensors`**。AI 圈从不停，但真正值得停下来看一眼的节点通常会改写工具、成本和想象力。
...
| 一手入口 | [阅读原文](https://github.com/huggingface/transformers/releases/tag/v4.23.0) |
| 收录月 | 2022-10 |
...
- **公开信息：** Add WhisperModel to transformers by @ArthurZucker in 19166
...
- **公开信息：** Add TF whisper by @amyeroberts in 19378
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
...
- **公开信息：** Add Deformable DETR by @NielsRogge in 17281
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
...
- **公开信息：** [fix] Add DeformableDetrFeatureExtractor by @NielsRogge in 19140
...
- **公开信息：** Add support for conditional detr by @DeppMeng in 18948
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
- **公开信息：** Improve conditional detr docs by @NielsRogge in 19154
...
- **公开信息：** time series forecasting model by @kashif in 17965
...
- **公开信息：** MSN (Masked Siamese Networks) for ViT by @sayakpaul in 18815
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
对「Hugging Face Transformers v4.23.0: Whisper, Deformable DETR, Conditional DETR, MarkupLM, MSN, `safetensors`」做专业诊断时，先定位它主要改写的是结构、激励，还是约束。三者抓错，后续讨论会全程错位。

请用下面的分析骨架阅读「Hugging Face Transformers v4.23.0: Whisper, Deformable DETR, Conditional DETR, MarkupLM, MSN, `safetensors`」，它比单靠情绪词汇更接近机构研报的工作方式：
