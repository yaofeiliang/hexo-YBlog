---
title: "[行业观察] Hugging Face Transformers v4.18.0: Checkpoint sharding, vision models：给非研究员的实用导读"
date: 2022-04-07 10:00:00
updated: 2022-04-07 10:00:00
description: "人工智能｜Hugging Face Transformers v4.18.0: Checkpoint sharding, vision models。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2022/ai-hugging-face-transformers-hugging-face-transformers-v4-18-0-/
categories:
  - 资讯
tags:
  - 行业资讯
...
Hugging Face Transformers 在 **2022-04-07** 留下可核对记录：**Hugging Face Transformers v4.18.0: Checkpoint sharding, vision models**。AI 圈从不停，但真正值得停下来看一眼的节点通常会改写工具、成本和想象力。
...
| 一手入口 | [阅读原文](https://github.com/huggingface/transformers/releases/tag/v4.18.0) |
| 收录月 | 2022-04 |
...
- **公开信息：** Add GLPN by @NielsRogge in https://github.com/huggingface/transformers/pull/16199
...
- **公开信息：** Resnet by @FrancescoSaverioZuppichini in https://github.com/huggingface/transformers/pull/15770
...
- **公开信息：** Visual Attention Network (VAN) by @FrancescoSaverioZuppichini in https://github.com/huggingface/transformers/pull/16027
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** add VisionTextDualEncoder and CLIP fine-tuning script by @patil-suraj in https://github.com/huggingface/transformers/pull/15701
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
...
- **公开信息：** document image classification: the RVL-CDIP dataset (a collection of 400,000 images belonging to one of 16 classes).
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 失败或误解的代价由谁承担：用户、平台，还是机构？
- **可执行翻译：** 若相关，就开一个 30 分钟验证任务；若不相关，也明确写“暂不跟”。
...
- **公开信息：** document layout analysis: the PubLayNet dataset (a collection of more than 360,000 document images constructed by automatically parsing PubMed XML files).
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
- **公开信息：** table detection: the ICDAR 2019 cTDaR dataset (a collection of 600 training images and 240 testing images).
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
- **公开信息：** Add Document Image Transformer (DiT) by @NielsRogge in https://github.com/huggingface/transformers/pull/15984
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
请用下面的分析骨架阅读「Hugging Face Transformers v4.18.0: Checkpoint sharding, vision models」，它比单靠情绪词汇更接近机构研报的工作方式：
