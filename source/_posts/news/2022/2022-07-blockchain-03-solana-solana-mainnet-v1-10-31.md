---
title: "[行业观察] Solana Mainnet - v1.10.31：把链上黑话翻译成人话"
date: 2022-07-12 10:00:00
updated: 2022-07-12 10:00:00
description: "区块链｜Solana Mainnet - v1.10.31。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2022/blockchain-solana-solana-mainnet-v1-10-31/
categories:
  - 资讯
tags:
  - 行业资讯
...
Solana 在 **2022-07-12** 留下可核对记录：**Solana Mainnet - v1.10.31**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 一手入口 | [阅读原文](https://github.com/solana-labs/solana/releases/tag/v1.10.31) |
| 收录月 | 2022-07 |
...
- **公开信息：** Use client certs in QUIC to get peer's stake (backport 26477) (26567)
...
- **公开信息：** Quic handshake timeout (backport 26306) (26563)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
...
- **公开信息：** Remove sender stakes from bankingstage buffer prioritization (backport 26512) (26562)
...
- **公开信息：** preserves rentepoch for rent exempt accounts (backport 26479) (26518)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
...
- **公开信息：** remote-wallet: add usb pids for ledger nano s plus (backport 26520) (26523)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
- **公开信息：** Refactor ConnectionCache::usequic (backport 26235) (26503)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 失败或误解的代价由谁承担：用户、平台，还是机构？
- **可执行翻译：** 若相关，就开一个 30 分钟验证任务；若不相关，也明确写“暂不跟”。
...
- **公开信息：** client: Use async connection in async TPU client (backport 25969) (26505)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
- **公开信息：** Cleanup repair logging (backport 26461) (26484)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
请用下面的分析骨架阅读「Solana Mainnet - v1.10.31」，它比单靠情绪词汇更接近机构研报的工作方式：
