---
title: "[行业观察] go-ethereum Sharblu (v1.10.18)：升级、托管与退出路径"
date: 2022-05-25 10:00:00
updated: 2022-05-25 10:00:00
description: "区块链｜go-ethereum Sharblu (v1.10.18)。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2022/blockchain-go-ethereum-go-ethereum-sharblu-v1-10-18/
categories:
  - 资讯
tags:
  - 行业资讯
...
go-ethereum 在 **2022-05-25** 留下可核对记录：**go-ethereum Sharblu (v1.10.18)**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 一手入口 | [阅读原文](https://github.com/ethereum/go-ethereum/releases/tag/v1.10.18) |
| 收录月 | 2022-05 |
...
- **公开信息：** Ropsten: terminal total difficulty is configured for the Merge. (24876)
...
- **公开信息：** In RPC APIs where a block number can be given, you can now use "finalized" to refer to the latest finalized block. (24282)
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** Geth can now serve CL requests to build a new block with very low latency because new blocks are constructed in the background. (24866)
...
- **公开信息：** For CL-induced reorgs to a block with unavailable state, Geth now recomputes the state by re-processing ancestor blocks. (24613)
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** The "engine" API implementation complies with the latest specification version. (24802, 24855, 24915)
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
- **公开信息：** Several annoyances related to sync after the Merge have been fixed. (24691, 24670, 24610)
...
- **公开信息：** Geth's default gas limit target is now 30M. (24680)
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 失败或误解的代价由谁承担：用户、平台，还是机构？
- **可执行翻译：** 若相关，就开一个 30 分钟验证任务；若不相关，也明确写“暂不跟”。
...
- **公开信息：** Geth now prints a warning when launching on the deprecated Rinkeby testnet. (24884)
...
对「go-ethereum Sharblu (v1.10.18)」做专业诊断时，先定位它主要改写的是结构、激励，还是约束。三者抓错，后续讨论会全程错位。

请用下面的分析骨架阅读「go-ethereum Sharblu (v1.10.18)」，它比单靠情绪词汇更接近机构研报的工作方式：
