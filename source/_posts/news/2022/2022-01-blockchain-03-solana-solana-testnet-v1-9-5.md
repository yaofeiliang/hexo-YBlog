---
title: "[行业观察] Solana Testnet - v1.9.5：先看规则，再看热闹"
date: 2022-01-22 10:00:00
updated: 2022-01-22 10:00:00
description: "区块链｜Solana Testnet - v1.9.5。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2022/blockchain-solana-solana-testnet-v1-9-5/
categories:
  - 资讯
tags:
  - 行业资讯
...
source_url: https://github.com/solana-labs/solana/releases/tag/v1.9.5
...
Solana 在 **2022-01-22** 留下可核对记录：**Solana Testnet - v1.9.5**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 时间 | 2022-01-22 |
| 主体 | Solana |
| 事件 | Solana Testnet - v1.9.5 |
| 一手入口 | [阅读原文](https://github.com/solana-labs/solana/releases/tag/v1.9.5) |
| 收录月 | 2022-01 |
...
- **公开信息：** Improves ReadOnlyAccountsCache performance
...
- **公开信息：** Disallows creation of rent-liable accounts
...
- **公开信息：** Caches deserialized Sysvars
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** Accounts for Vote TX CUs when packing blocks
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** Allows Vote account withdraw authority to change vote authority
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
- **公开信息：** Improves SigVerify drop performance (thanks to @buffalu)
- **阅读提醒：** 性能信号：必须落到你们的基准场景（P95 延迟、构建时长、包体、内存）复测，避免被形容词绑架。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
- **公开信息：** Improvse SigVerify discard performance
- **阅读提醒：** 性能信号：必须落到你们的基准场景（P95 延迟、构建时长、包体、内存）复测，避免被形容词绑架。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
- **公开信息：** Improves SigVerify TX dedupe performance
- **阅读提醒：** 性能信号：必须落到你们的基准场景（P95 延迟、构建时长、包体、内存）复测，避免被形容词绑架。
- **我想追问的：** 失败或误解的代价由谁承担：用户、平台，还是机构？
- **可执行翻译：** 若相关，就开一个 30 分钟验证任务；若不相关，也明确写“暂不跟”。
...
对「Solana Testnet - v1.9.5」做专业诊断时，先定位它主要改写的是结构、激励，还是约束。三者抓错，后续讨论会全程错位。

请用下面的分析骨架阅读「Solana Testnet - v1.9.5」，它比单靠情绪词汇更接近机构研报的工作方式：
...
- 原始日期：2022-01-22
- 收录月份：2022-01
- 一手链接：<https://github.com/solana-labs/solana/releases/tag/v1.9.5>
