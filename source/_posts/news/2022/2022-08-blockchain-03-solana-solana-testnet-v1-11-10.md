---
title: "[行业观察] Solana Testnet - v1.11.10：比行情截图更值得读的部分"
date: 2022-08-26 10:00:00
updated: 2022-08-26 10:00:00
description: "区块链｜Solana Testnet - v1.11.10。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2022/blockchain-solana-solana-testnet-v1-11-10/
categories:
  - 资讯
tags:
  - 行业资讯
...
source_url: https://github.com/solana-labs/solana/releases/tag/v1.11.10
source_published: 2022-08-26
...
Solana 在 **2022-08-26** 留下可核对记录：**Solana Testnet - v1.11.10**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 时间 | 2022-08-26 |
| 主体 | Solana |
| 事件 | Solana Testnet - v1.11.10 |
| 一手入口 | [阅读原文](https://github.com/solana-labs/solana/releases/tag/v1.11.10) |
| 收录月 | 2022-08 |
...
- **公开信息：** Address RUSTSEC-2022-0051
...
- **公开信息：** Verify snapshot slot deltas (backport 26666) (27373)
...
- **公开信息：** Fix quic staked chunking (backport 27402) (27408)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
...
- **公开信息：** Handle VoteStateUpdates for outdated roots bigger than slots in existing VoteState (backport 27323) (27392)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
...
- **公开信息：** coalesce entries in recvslotentries to target byte count (backport 27321) (27407)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 失败或误解的代价由谁承担：用户、平台，还是机构？
- **可执行翻译：** 若相关，就开一个 30 分钟验证任务；若不相关，也明确写“暂不跟”。
...
- **公开信息：** Check overflow on vote tx compaction boundary (backport 27185) (27359)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
- **公开信息：** only READ BankIncrementalSnapshotPersistence in 1.11 (27345)
...
- **公开信息：** serialize incrementalsnapshothash (backport 26839) (27212)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
请用下面的分析骨架阅读「Solana Testnet - v1.11.10」，它比单靠情绪词汇更接近机构研报的工作方式：
...
- 原始日期：2022-08-26
- 收录月份：2022-08
- 一手链接：<https://github.com/solana-labs/solana/releases/tag/v1.11.10>
