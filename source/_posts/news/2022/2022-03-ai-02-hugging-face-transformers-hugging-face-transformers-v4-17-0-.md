---
title: "[行业观察] Hugging Face Transformers v4.17.0: XGLM, ConvNext, PoolFormer, PLBart, Data2Vec, MaskFormer and code in the Hub：这次到底方便了谁"
date: 2022-03-03 10:00:00
updated: 2022-03-03 10:00:00
description: "人工智能｜Hugging Face Transformers v4.17.0: XGLM, ConvNext, PoolFormer, PLBart, Data2Vec, MaskFormer and code in the Hub。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2022/ai-hugging-face-transformers-hugging-face-transformers-v4-17-0-/
categories:
  - 资讯
tags:
  - 行业资讯
...
Hugging Face Transformers 在 **2022-03-03** 留下可核对记录：**Hugging Face Transformers v4.17.0: XGLM, ConvNext, PoolFormer, PLBart, Data2Vec, MaskFormer and code in the Hub**。AI 圈从不停，但真正值得停下来看一眼的节点通常会改写工具、成本和想象力。
...
| 一手入口 | [阅读原文](https://github.com/huggingface/transformers/releases/tag/v4.17.0) |
| 收录月 | 2022-03 |
...
- **公开信息：** Add XGLM models by @patil-suraj in https://github.com/huggingface/transformers/pull/14876
...
- **公开信息：** Add ConvNeXT by @NielsRogge in https://github.com/huggingface/transformers/pull/15277
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
...
- **公开信息：** Add TFConvNextModel by @sayakpaul in https://github.com/huggingface/transformers/pull/15750
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
...
- **公开信息：** Add PoolFormer by @heytanay in https://github.com/huggingface/transformers/pull/15531
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
...
- **公开信息：** Add PLBart by @gchhablani in https://github.com/huggingface/transformers/pull/13269
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
- **公开信息：** Add missing PLBart entry in README by @gchhablani in https://github.com/huggingface/transformers/pull/15721
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
- **公开信息：** Add Data2Vec by @edugp in https://github.com/huggingface/transformers/pull/15507
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **我想追问的：** 失败或误解的代价由谁承担：用户、平台，还是机构？
- **可执行翻译：** 若相关，就开一个 30 分钟验证任务；若不相关，也明确写“暂不跟”。
...
- **公开信息：** Maskformer by @FrancescoSaverioZuppichini in https://github.com/huggingface/transformers/pull/15682
...
对「Hugging Face Transformers v4.17.0: XGLM, ConvNext, PoolFormer, PLBart, Data2Vec, MaskFormer and code in the Hub」做专业诊断时，先定位它主要改写的是结构、激励，还是约束。三者抓错，后续讨论会全程错位。

请用下面的分析骨架阅读「Hugging Face Transformers v4.17.0: XGLM, ConvNext, PoolFormer, PLBart, Data2Vec, MaskFormer and code in the Hub」，它比单靠情绪词汇更接近机构研报的工作方式：
