---
title: "[行业观察] Solana Testnet - v1.14.9 (not compatible with mainnet-beta or devnet!)：把链上黑话翻译成人话"
date: 2022-11-28 10:00:00
updated: 2022-11-28 10:00:00
description: "区块链｜Solana Testnet - v1.14.9 (not compatible with mainnet-beta or devnet!)。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2022/blockchain-solana-solana-testnet-v1-14-9-not-compatible-with-mainnet-be/
categories:
  - 资讯
tags:
  - 行业资讯
...
Solana 在 **2022-11-28** 留下可核对记录：**Solana Testnet - v1.14.9 (not compatible with mainnet-beta or devnet!)**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 一手入口 | [阅读原文](https://github.com/solana-labs/solana/releases/tag/v1.14.9) |
| 收录月 | 2022-11 |
...
- **公开信息：** feature: on accounts hash calculation, do not try to rehash accounts (28935)
...
- **公开信息：** dedups turbine retransmit peers by tvu socket addresses (backport 28944) (28963)
...
- **公开信息：** update to spl-token-2022 0.5.0
...
- **公开信息：** rolls out merkle shreds to ~20% of testnet (backport 28905) (28907)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
...
- **公开信息：** Sort offline/wrong-shred nodes by stake weight while waiting for supermajority (backport 28872) (28886)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
- **公开信息：** Revert "Cap accounts data a transaction can load by its requested limit (backport 27840)" (28861)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 失败或误解的代价由谁承担：用户、平台，还是机构？
- **可执行翻译：** 若相关，就开一个 30 分钟验证任务；若不相关，也明确写“暂不跟”。
...
- **公开信息：** Cap accounts data a transaction can load by its requested limit (backport 27840) (28797)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
- **公开信息：** Fix client getprogramaccountswithconfig calls with context (backport 28772) (28847)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
对「Solana Testnet - v1.14.9 (not compatible with mainnet-beta or devnet!)」做专业诊断时，先定位它主要改写的是结构、激励，还是约束。三者抓错，后续讨论会全程错位。

请用下面的分析骨架阅读「Solana Testnet - v1.14.9 (not compatible with mainnet-beta or devnet!)」，它比单靠情绪词汇更接近机构研报的工作方式：
