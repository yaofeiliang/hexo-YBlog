---
title: "[行业观察] Solana Testnet - v1.9.7：比行情截图更值得读的部分"
date: 2022-02-17 10:00:00
updated: 2022-02-17 10:00:00
description: "区块链｜Solana Testnet - v1.9.7。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2022/blockchain-solana-solana-testnet-v1-9-7/
categories:
  - 资讯
tags:
  - 行业资讯
...
Solana 在 **2022-02-17** 留下可核对记录：**Solana Testnet - v1.9.7**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 一手入口 | [阅读原文](https://github.com/solana-labs/solana/releases/tag/v1.9.7) |
| 收录月 | 2022-02 |
...
- **公开信息：** Fixes UDP port allocation ordering
...
- **公开信息：** Fixes snapshot serving under minimal RPC api
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
...
- **公开信息：** Fixes consensus incompatibility with v1.8 branch handling ed25519 precompiles
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
...
- **公开信息：** CLI - Fixes changing stake account staking authority with withdraw authority
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
...
- **公开信息：** Adds additional fee specification compute budget
...
- **公开信息：** Adds packet batch shrinking when 80+% of packets are discarded
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
- **公开信息：** Improves validator management tools (monitor and wait-for-restart-window)
...
- **公开信息：** Improves metrics and documentation
...
请用下面的分析骨架阅读「Solana Testnet - v1.9.7」，它比单靠情绪词汇更接近机构研报的工作方式：
