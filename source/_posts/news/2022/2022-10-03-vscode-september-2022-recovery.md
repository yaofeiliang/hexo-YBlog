---
title: "[技术观察] Visual Studio Code September 2022 Recovery：别只看版本号，先看你会不会被影响"
date: 2022-10-11 09:00:00
updated: 2022-10-11 09:00:00
description: "Visual Studio Code 于 2022-10-11 发布 September 2022 Recovery。结合官方条目、场景推演与升级检查，帮你判断要不要跟、怎么跟。"
...
又一条值得记账的发布：Visual Studio Code September 2022 Recovery（2022-10-11）。标题可以热闹，决策还是得冷静。

若压缩成决策语言： Visual Studio Code 的发布页是事实源头；我做的是把英文/术语密集的说明拆成可以讨论、可以排期、可以回滚的中文导读。带“官方”字样的内容来自发布页；场景和检查清单是通用工作方法，不表示某一条一定是本版本新增能力。

这不是人人都要半夜升级的警报。真正相关的是已经把 Visual Studio Code 写进构建、部署或日常开发链路的人；其他人把它当行业动态就好。

一手入口在这里：<https://github.com/microsoft/vscode/releases/tag/1.72.1>
...
| 发布名称 | September 2022 Recovery |
| 版本标签 | 1.72.1 |
| 原始发布日期 | 2022-10-11 |
| 是否预发布 | 否（正式发布版本） |
| 仓库 | microsoft/vscode |
| 官方发布页 | [查看原文](https://github.com/microsoft/vscode/releases/tag/1.72.1) |
...
- **官方原文：** The update addresses these issues, including a fix for a security vulnerability.
  - **怎么读：** 安全信号：优先核对受影响组件与暴露面，再决定是热修、灰度还是冻结发布窗口。
- **官方原文：** For the complete release notes go to Updates on code.visualstudio.com.
...
对 Visual Studio Code（September 2022 Recovery / 1.72.1），建议按下列层面对齐阅读，而不是只扫版本号：

- **机制层（它可能触达的子系统）：** 编辑器核心、语言服务、扩展主机、调试适配、远程/容器开发链路
- **约束层（最常见的工程风险）：** 扩展 API 不兼容、语言服务回归、远程连接中断、团队配置同步放大故障
- **证据层（升级后应用哪些指标说话）：** 扩展激活失败、调试启动成功率、远程会话稳定性、关键工作流耗时
...
假设配置通过 Settings Sync 统一：应先小流量验证，再全员推送，避免“共享配置一次性放大故障”。 在这个场景中，广度分析的价值是防止“局部最优”：单个仓库升级成功，不代表系统级风险下降。专业团队看的是组合风险，而不是单点绿勾。
...
软件版本常见写法是“主版本.次版本.修订版本”。最后一位变化，很多项目用来修问题；中间一位可能带来可见的新能力；最前面一位变化，兼容性风险通常最大。但不同项目纪律不同——Visual Studio Code 也不例外——所以数字只是线索，说明文字才是证据。

有个笨办法非常管用：先写“当前版本、目标版本、验证清单、失败如何回去”，再动手。

单独分支或预发环境装新版本 → 跑构建与关键路径 → 没问题再合入主干。听起来慢，其实比线上紧急回滚快。

阅读 Visual Studio Code 时，这几个词会反复出现：
...
以下清单是面向 Visual Studio Code 的**通用建议**，用来降低升级翻车概率；它不是对本版本功能的逐条断言：

1. 编辑器功能与扩展兼容性 —— 请写成可勾选的验证步骤，而不是停留在口头提醒。
2. 远程开发、调试和语言服务体验 —— 请写成可勾选的验证步骤，而不是停留在口头提醒。
3. 团队设置同步与稳定性 —— 请写成可勾选的验证步骤，而不是停留在口头提醒。
...
假设配置通过 Settings Sync 统一：应先小流量验证，再全员推送，避免“共享配置一次性放大故障”。 把上面四个问题套进去，你会立刻知道自己缺的是信息、测试，还是决策人。
...
你可以直接反对我的权重分配，但请把反对写成可检验命题——那才是研究，而不是情绪。 下一回再遇到 Visual Studio Code 的发布，你就不是从零开始，而是在更新一张已经存在的风险地图。
