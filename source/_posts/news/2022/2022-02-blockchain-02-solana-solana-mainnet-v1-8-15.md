---
title: "[行业观察] Solana Mainnet - v1.8.15：比行情截图更值得读的部分"
date: 2022-02-18 10:00:00
updated: 2022-02-18 10:00:00
description: "区块链｜Solana Mainnet - v1.8.15。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2022/blockchain-solana-solana-mainnet-v1-8-15/
categories:
  - 资讯
tags:
  - 行业资讯
...
source_url: https://github.com/solana-labs/solana/releases/tag/v1.8.15
source_published: 2022-02-18
...
Solana 在 **2022-02-18** 留下可核对记录：**Solana Mainnet - v1.8.15**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 时间 | 2022-02-18 |
| 主体 | Solana |
| 事件 | Solana Mainnet - v1.8.15 |
| 一手入口 | [阅读原文](https://github.com/solana-labs/solana/releases/tag/v1.8.15) |
| 收录月 | 2022-02 |
...
- **公开信息：** Allows cli users to authorize Staker signed by Withdrawer
...
- **公开信息：** Removes cost model from end of slot filtering
...
- **公开信息：** Rejects close of active vote accounts
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** Updates syscall base costs
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** Implements json output for solana ping
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 失败或误解的代价由谁承担：用户、平台，还是机构？
- **可执行翻译：** 若相关，就开一个 30 分钟验证任务；若不相关，也明确写“暂不跟”。
...
- **公开信息：** Simplifies ShredIndex api
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
- **公开信息：** Uses RPC minimal mode by default
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
- **公开信息：** Adds sanity checks to shreds
...
请用下面的分析骨架阅读「Solana Mainnet - v1.8.15」，它比单靠情绪词汇更接近机构研报的工作方式：
...
- 原始日期：2022-02-18
- 收录月份：2022-02
- 一手链接：<https://github.com/solana-labs/solana/releases/tag/v1.8.15>
