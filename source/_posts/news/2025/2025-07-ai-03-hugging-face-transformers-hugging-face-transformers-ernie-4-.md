---
title: "[行业观察] Hugging Face Transformers Ernie-4.5 and Ernie-4.5 MoE (based on v4.53.2)：这次到底方便了谁"
date: 2025-07-23 10:00:00
updated: 2025-07-23 10:00:00
description: "人工智能｜Hugging Face Transformers Ernie-4.5 and Ernie-4.5 MoE (based on v4.53.2)。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2025/ai-hugging-face-transformers-hugging-face-transformers-ernie-4-/
categories:
  - 资讯
tags:
  - 行业资讯
...
Hugging Face Transformers 在 **2025-07-23** 留下可核对记录：**Hugging Face Transformers Ernie-4.5 and Ernie-4.5 MoE (based on v4.53.2)**。AI 圈从不停，但真正值得停下来看一眼的节点通常会改写工具、成本和想象力。
...
| 一手入口 | [阅读原文](https://github.com/huggingface/transformers/releases/tag/v4.53.2-Ernie-4.5-preview) |
| 收录月 | 2025-07 |
...
- **公开信息：** Two new models are added to transformers: Ernie 4.5, and its MoE variant, Ernie 4.5 MoE.
...
- **公开信息：** They are added on top of the v4.53.2 release, and can be installed from the following tag: v4.53.2-Ernie-4.5-preview.
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
...
- **公开信息：** In order to install this version, please install with the following command: pip install git+https://github.com/huggingface/transformers@v4.53.2-Ernie-4.5-preview If fixes are needed, they will be applied to this release; this installation may therefore be considered as stable and improving.
...
- **公开信息：** As the tag implies, this tag is a preview of the Ernie-4.5 models.
...
- **公开信息：** This model will be included in the next minor release: v4.54.0.
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
请用下面的分析骨架阅读「Hugging Face Transformers Ernie-4.5 and Ernie-4.5 MoE (based on v4.53.2)」，它比单靠情绪词汇更接近机构研报的工作方式：
