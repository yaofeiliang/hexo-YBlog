---
title: "[行业观察] Solana Testnet - v1.16.11：先看规则，再看热闹"
date: 2023-08-31 10:00:00
updated: 2023-08-31 10:00:00
description: "区块链｜Solana Testnet - v1.16.11。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2023/blockchain-solana-solana-testnet-v1-16-11/
categories:
  - 资讯
tags:
  - 行业资讯
...
Solana 在 **2023-08-31** 留下可核对记录：**Solana Testnet - v1.16.11**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 一手入口 | [阅读原文](https://github.com/solana-labs/solana/releases/tag/v1.16.11) |
| 收录月 | 2023-08 |
...
- **公开信息：** In previous v1.16 releases, the TransactionStatus and AddressSignatures columns were not properly cleaned. This could have resulted in excessive storage use for nodes running with --enable-rpc-transaction-history. This has been fixed with https://github.com/solana-labs/solana/pull/32548
...
- **公开信息：** v1.16: [zk-token-proof] Enable proof program to be invoked as an inner call (backport of 33045) (33071)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
...
- **公开信息：** v1.16: [zk-token-proof] Add CUs to close account instruction (backport of 33042) (33075)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
...
- **公开信息：** v1.16: unpin tokio for various client crates (33058)
...
- **公开信息：** v1.16: rekey stakeredelegateinstruction feature (backport of 33056) (33059)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
- **公开信息：** v1.16: banks-server: Support versioned transactions (backport of 33005) (33036)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
- **公开信息：** v1.16: revises turbine epoch stakes for shreds propagation (backport of 32743) (33041)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
- **公开信息：** v1.16: Handle closing of the program in the same slot as deployment (backport of 33025) (33027)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 失败或误解的代价由谁承担：用户、平台，还是机构？
- **可执行翻译：** 若相关，就开一个 30 分钟验证任务；若不相关，也明确写“暂不跟”。
...
对「Solana Testnet - v1.16.11」做专业诊断时，先定位它主要改写的是结构、激励，还是约束。三者抓错，后续讨论会全程错位。

请用下面的分析骨架阅读「Solana Testnet - v1.16.11」，它比单靠情绪词汇更接近机构研报的工作方式：
