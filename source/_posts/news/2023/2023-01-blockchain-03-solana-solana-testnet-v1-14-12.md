---
title: "[行业观察] Solana Testnet - v1.14.12：先看规则，再看热闹"
date: 2023-01-10 10:00:00
updated: 2023-01-10 10:00:00
description: "区块链｜Solana Testnet - v1.14.12。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2023/blockchain-solana-solana-testnet-v1-14-12/
categories:
  - 资讯
tags:
  - 行业资讯
...
Solana 在 **2023-01-10** 留下可核对记录：**Solana Testnet - v1.14.12**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 一手入口 | [阅读原文](https://github.com/solana-labs/solana/releases/tag/v1.14.12) |
| 收录月 | 2023-01 |
...
- **公开信息：** Fix bank hash mismatch between v1.13.5 and v1.14.11 (29593)
...
- **公开信息：** Add rust client support for getRecentPrioritizationFees (backport 29558) (29564)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
...
- **公开信息：** Add getRecentPrioritizationFees RPC endpoint (backport 27278) (29562)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
...
- **公开信息：** collect min prioritization fees when replaying sanitized transactions (backport 26709) (29539)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
...
- **公开信息：** require repair request signature, ping/pong for Testnet, Development clusters (backport 29351) (29529)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
- **公开信息：** Add a SanitizedMessage type that caches writable accounts indexes (backport 27317) (29538)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
- **公开信息：** solana-install: check for fixed releases directly (backport 29365) (29520)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
- **公开信息：** dedups gossip addresses, taking the one with highest weight (backport 29421) (29493)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 失败或误解的代价由谁承担：用户、平台，还是机构？
- **可执行翻译：** 若相关，就开一个 30 分钟验证任务；若不相关，也明确写“暂不跟”。
...
请用下面的分析骨架阅读「Solana Testnet - v1.14.12」，它比单靠情绪词汇更接近机构研报的工作方式：
