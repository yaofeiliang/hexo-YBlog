---
title: "[行业观察] go-ethereum Annos Basin (v1.11.0)：升级、托管与退出路径"
date: 2023-02-15 10:00:00
updated: 2023-02-15 10:00:00
description: "区块链｜go-ethereum Annos Basin (v1.11.0)。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2023/blockchain-go-ethereum-go-ethereum-annos-basin-v1-11-0/
categories:
  - 资讯
tags:
  - 行业资讯
...
go-ethereum 在 **2023-02-15** 留下可核对记录：**go-ethereum Annos Basin (v1.11.0)**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 一手入口 | [阅读原文](https://github.com/ethereum/go-ethereum/releases/tag/v1.11.0) |
| 收录月 | 2023-02 |
...
- **公开信息：** Stick with goleveldb as long as we can, don't worry about it,
...
- **公开信息：** Fork goleveldb (or some other database), and maintain it ourselves.
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** Pick a different database, actively maintained by a dedicated team.
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** You need to be on a 64-bit system,
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** You also need to resync from scratch (with or without ancients) -- there is no migration functionality,
...
- **公开信息：** And you need to specify --db.engine=pebble initially. For subsequent runs, geth should discover pebble automatically.
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
请用下面的分析骨架阅读「go-ethereum Annos Basin (v1.11.0)」，它比单靠情绪词汇更接近机构研报的工作方式：
