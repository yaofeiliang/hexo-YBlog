---
title: "[行业观察] go-ethereum Spekilas (v1.11.3)：比行情截图更值得读的部分"
date: 2023-03-07 10:00:00
updated: 2023-03-07 10:00:00
description: "区块链｜go-ethereum Spekilas (v1.11.3)。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2023/blockchain-go-ethereum-go-ethereum-spekilas-v1-11-3/
categories:
  - 资讯
tags:
  - 行业资讯
...
go-ethereum 在 **2023-03-07** 留下可核对记录：**go-ethereum Spekilas (v1.11.3)**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 一手入口 | [阅读原文](https://github.com/ethereum/go-ethereum/releases/tag/v1.11.3) |
| 收录月 | 2023-03 |
...
- **公开信息：** You can now set more than 4 GB in --cache when using Pebble. (26776)
...
- **公开信息：** Range compaction now works correctly for Pebble. (26771)
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** Pebble support is disabled on OpenBSD to resolve a compilation error. (26801)
...
- **公开信息：** Add support for Parity-style flat traces with the new built-in flatCallTracer. (26377)
...
- **公开信息：** The callTracer now reports a null address for failed contract creation operations. (26779)
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **我想追问的：** 失败或误解的代价由谁承担：用户、平台，还是机构？
- **可执行翻译：** 若相关，就开一个 30 分钟验证任务；若不相关，也明确写“暂不跟”。
...
- **公开信息：** head and difficulty have been removed in adminpeerInfo responses. (26804)
...
- **公开信息：** types.Receipt now contains the EffectiveGasPrice of the transaction, so you can get the true gas price using the TransactionReceipt method of ethclient. (26713)
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
- **公开信息：** ethclient no longer panics when requesting missing blocks. This fixes a regression introduced in v1.11.2. (26817)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
对「go-ethereum Spekilas (v1.11.3)」做专业诊断时，先定位它主要改写的是结构、激励，还是约束。三者抓错，后续讨论会全程错位。

请用下面的分析骨架阅读「go-ethereum Spekilas (v1.11.3)」，它比单靠情绪词汇更接近机构研报的工作方式：
