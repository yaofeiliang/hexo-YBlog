---
title: "[行业观察] go-ethereum Kite's Nest (v1.11.2)：把链上黑话翻译成人话"
date: 2023-02-22 10:00:00
updated: 2023-02-22 10:00:00
description: "区块链｜go-ethereum Kite's Nest (v1.11.2)。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2023/blockchain-go-ethereum-go-ethereum-kite-s-nest-v1-11-2/
categories:
  - 资讯
tags:
  - 行业资讯
...
go-ethereum Kite's Nest (v1.11.2) 这类节点，适合当成“制度/协议变更”，而不是当成心情天气。

go-ethereum 在 **2023-02-22** 留下可核对记录：**go-ethereum Kite's Nest (v1.11.2)**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 一手入口 | [阅读原文](https://github.com/ethereum/go-ethereum/releases/tag/v1.11.2) |
| 收录月 | 2023-02 |
...
- **公开信息：** Fix a few small engine API discordances with the spec post-Shanghai (26696, 26722).
...
- **公开信息：** Fix unmarshalling JSON null values as a proper result instead of nil (26723).
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
...
- **公开信息：** Fix dumpgenesis which failed due to a bad database key write (26747).
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
...
- **公开信息：** Fix pending tx filter to return hashes, not full txs by default (26757).
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
...
- **公开信息：** Fix ethfeeHistory to accept decimal blocks again (26758).
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
- **公开信息：** Fix Ubuntu PPA builds after the Go 1.20 fallout.
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 失败或误解的代价由谁承担：用户、平台，还是机构？
- **可执行翻译：** 若相关，就开一个 30 分钟验证任务；若不相关，也明确写“暂不跟”。
...
请用下面的分析骨架阅读「go-ethereum Kite's Nest (v1.11.2)」，它比单靠情绪词汇更接近机构研报的工作方式：
