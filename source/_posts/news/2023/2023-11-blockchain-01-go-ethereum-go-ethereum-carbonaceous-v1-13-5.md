---
title: "[行业观察] go-ethereum Carbonaceous (v1.13.5)：比行情截图更值得读的部分"
date: 2023-11-14 10:00:00
updated: 2023-11-14 10:00:00
description: "区块链｜go-ethereum Carbonaceous (v1.13.5)。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2023/blockchain-go-ethereum-go-ethereum-carbonaceous-v1-13-5/
categories:
  - 资讯
tags:
  - 行业资讯
...
go-ethereum Carbonaceous (v1.13.5) 这类节点，适合当成“制度/协议变更”，而不是当成心情天气。

go-ethereum 在 **2023-11-14** 留下可核对记录：**go-ethereum Carbonaceous (v1.13.5)**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 一手入口 | [阅读原文](https://github.com/ethereum/go-ethereum/releases/tag/v1.13.5) |
| 收录月 | 2023-11 |
...
- **公开信息：** Extend ethclient and the simulated backend to allow ethcall against specific block hashes (28084).
...
- **公开信息：** Downgrade annoying stale transaction propagation logs from warning to debug (28364).
...
- **公开信息：** Switch to the new KZG trusted setup parameters (28383).
...
- **公开信息：** Return an error on GraphQL if querying invalid block ranges (28393, 28412).
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** Start publishing Apple Silicon pre-built binaries (28474, 28475).
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 失败或误解的代价由谁承担：用户、平台，还是机构？
- **可执行翻译：** 若相关，就开一个 30 分钟验证任务；若不相关，也明确写“暂不跟”。
...
- **公开信息：** Fix a number of corner-cases in path scheme state management (28198, 28426, 28483).
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
- **公开信息：** Fix an issue when allocating excessively large Pebble caches (28444).
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
- **公开信息：** Fix a potential snap sync issue with the path based storage (28327).
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
请用下面的分析骨架阅读「go-ethereum Carbonaceous (v1.13.5)」，它比单靠情绪词汇更接近机构研报的工作方式：
