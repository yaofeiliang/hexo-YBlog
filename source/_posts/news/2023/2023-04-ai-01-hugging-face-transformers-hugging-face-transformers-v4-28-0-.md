---
title: "[行业观察] Hugging Face Transformers v4.28.0: LLaMa, Pix2Struct, MatCha, DePlot, MEGA, NLLB-MoE, GPTBigCode：别被演示绕晕，先看成本和边界"
date: 2023-04-13 10:00:00
updated: 2023-04-13 10:00:00
description: "人工智能｜Hugging Face Transformers v4.28.0: LLaMa, Pix2Struct, MatCha, DePlot, MEGA, NLLB-MoE, GPTBigCode。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2023/ai-hugging-face-transformers-hugging-face-transformers-v4-28-0-/
categories:
  - 资讯
tags:
  - 行业资讯
...
2023-04-13 的「Hugging Face Transformers v4.28.0: LLaMa, Pix2Struct, MatCha, DePlot, MEGA, NLLB-MoE, GPTBigCode」值得停一下。AI 圈从不缺口号，缺的是把边界讲清楚。

Hugging Face Transformers 在 **2023-04-13** 留下可核对记录：**Hugging Face Transformers v4.28.0: LLaMa, Pix2Struct, MatCha, DePlot, MEGA, NLLB-MoE, GPTBigCode**。AI 圈从不停，但真正值得停下来看一眼的节点通常会改写工具、成本和想象力。
...
| 一手入口 | [阅读原文](https://github.com/huggingface/transformers/releases/tag/v4.28.0) |
| 收录月 | 2023-04 |
...
- **公开信息：** LLaMA Implementation by @zphang in 21955
...
- **公开信息：** Add Pix2Struct by @younesbelkada in 21400
...
- **公开信息：** Add DePlot + MatCha on transformers by @younesbelkada in 22528
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
...
- **公开信息：** Add Mega: Moving Average Equipped Gated Attention by @mnaylor5 in 21766
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
...
- **公开信息：** Add GPTBigCode model (Optimized GPT2 with MQA from Santacoder & BigCode) by @jlamypoirier in 22575
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
- **公开信息：** NLLB-MoE Adds the moe model by @ArthurZucker in 22024
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
- **公开信息：** [bnb] Let's make serialization of int8 models possible by @younesbelkada in 22177
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
- **公开信息：** 🚨🚨🚨 Fix ordering of height, width for BLIP image processor by @amyeroberts in 22466
...
请用下面的分析骨架阅读「Hugging Face Transformers v4.28.0: LLaMa, Pix2Struct, MatCha, DePlot, MEGA, NLLB-MoE, GPTBigCode」，它比单靠情绪词汇更接近机构研报的工作方式：
