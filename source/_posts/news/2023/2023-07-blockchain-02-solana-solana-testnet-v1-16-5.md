---
title: "[行业观察] Solana Testnet - v1.16.5：比行情截图更值得读的部分"
date: 2023-07-22 10:00:00
updated: 2023-07-22 10:00:00
description: "区块链｜Solana Testnet - v1.16.5。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2023/blockchain-solana-solana-testnet-v1-16-5/
categories:
  - 资讯
tags:
  - 行业资讯
...
Solana 在 **2023-07-22** 留下可核对记录：**Solana Testnet - v1.16.5**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 一手入口 | [阅读原文](https://github.com/solana-labs/solana/releases/tag/v1.16.5) |
| 收录月 | 2023-07 |
...
- **公开信息：** The calculateaccountshashcache directory under ledger/ has been renamed. After upgrading to v1.16, the ledger/calculateaccountshashcache/ directory can be removed. https://github.com/solana-labs/solana/pull/29734
...
- **公开信息：** v1.16: Detect duplicates in the same insert batch (backport of 32528) (32557)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
...
- **公开信息：** v1.16: rolls out merkle shreds to ~20% of mainnet slots (backport of 32532) (32582)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
...
- **公开信息：** v1.16: Priority queue supports deprecated compute budget ix (backport of 32567) (32572)
- **阅读提醒：** 兼容性信号：旧接口/旧配置可能失效，应进入变更影响评估（impact assessment），而不是只做冒烟。
- **我想追问的：** 失败或误解的代价由谁承担：用户、平台，还是机构？
- **可执行翻译：** 若相关，就开一个 30 分钟验证任务；若不相关，也明确写“暂不跟”。
...
- **公开信息：** v1.16: Re-enable periodic compaction on several columns (backport of 32548) (32565)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
- **公开信息：** v1.16: enables merkle shreds for devnet and development clusters (backport of 32533) (32552)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
- **公开信息：** v1.16: allow shrinking to be in progress during scanaccountstorage (backport of 32553) (32556)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
请用下面的分析骨架阅读「Solana Testnet - v1.16.5」，它比单靠情绪词汇更接近机构研报的工作方式：
