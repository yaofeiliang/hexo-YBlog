---
title: "[行业观察] go-ethereum Erszbat (v1.11.5)：把链上黑话翻译成人话"
date: 2023-03-21 10:00:00
updated: 2023-03-21 10:00:00
description: "区块链｜go-ethereum Erszbat (v1.11.5)。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2023/blockchain-go-ethereum-go-ethereum-erszbat-v1-11-5/
categories:
  - 资讯
tags:
  - 行业资讯
...
source_url: https://github.com/ethereum/go-ethereum/releases/tag/v1.11.5
...
go-ethereum 在 **2023-03-21** 留下可核对记录：**go-ethereum Erszbat (v1.11.5)**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 时间 | 2023-03-21 |
| 主体 | go-ethereum |
| 事件 | go-ethereum Erszbat (v1.11.5) |
| 一手入口 | [阅读原文](https://github.com/ethereum/go-ethereum/releases/tag/v1.11.5) |
| 收录月 | 2023-03 |
...
- **公开信息：** EIP-3855: PUSH0 instruction
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** EIP-3860: Limit and meter initcode
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** EIP-4895: Beacon chain push withdrawals as operations
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** EIP-6049: Deprecate SELFDESTRUCT
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
- **公开信息：** Enable Shanghai at 1681338455 unix time (26908)
...
- **公开信息：** Improve error message on bad blocks (26870)
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
- **公开信息：** Preparatory changes for path-based storage (26763)
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
对「go-ethereum Erszbat (v1.11.5)」做专业诊断时，先定位它主要改写的是结构、激励，还是约束。三者抓错，后续讨论会全程错位。

请用下面的分析骨架阅读「go-ethereum Erszbat (v1.11.5)」，它比单靠情绪词汇更接近机构研报的工作方式：
...
- 原始日期：2023-03-21
- 收录月份：2023-03
- 一手链接：<https://github.com/ethereum/go-ethereum/releases/tag/v1.11.5>
