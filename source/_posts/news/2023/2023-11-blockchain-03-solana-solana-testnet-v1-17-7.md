---
title: "[行业观察] Solana Testnet - v1.17.7：比行情截图更值得读的部分"
date: 2023-11-30 10:00:00
updated: 2023-11-30 10:00:00
description: "区块链｜Solana Testnet - v1.17.7。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2023/blockchain-solana-solana-testnet-v1-17-7/
categories:
  - 资讯
tags:
  - 行业资讯
...
Solana 在 **2023-11-30** 留下可核对记录：**Solana Testnet - v1.17.7**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 一手入口 | [阅读原文](https://github.com/solana-labs/solana/releases/tag/v1.17.7) |
| 收录月 | 2023-11 |
...
- **公开信息：** The v1.17 minimum dynamic port range is now 14 so you may need to expand the range specified by --dynamic-port-range
...
- **公开信息：** This is expanded from the v1.16 minimum of 13
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** Archive formats[bz2, gzip, tar, none] have been deprecated for the --snapshot-archive-format argument (https://github.com/solana-labs/solana/pull/33484)
...
- **公开信息：** Existing snapshots with these formats can still be read, but solana-validator and solana-ledger-tool must be updated to use a different archive format. The recommended value is zstd, which is also the default if the --snapshot-archive-format argument is removed altogether
- **阅读提醒：** 兼容性信号：旧接口/旧配置可能失效，应进入变更影响评估（impact assessment），而不是只做冒烟。
...
- **公开信息：** v1.17: Allow Blockstore to open unknown columns (backport of 34174) (34287)
...
- **公开信息：** v1.17: Skip shrink until startup verification is complete (backport of 34209) (34227)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
- **公开信息：** v1.17: Output BankHashDetails file when leader drops its' own block (backport of 34256) (34275)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
- **公开信息：** v1.17: Increase pull request clusterinfo probability (backport of 34231) (34262)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
请用下面的分析骨架阅读「Solana Testnet - v1.17.7」，它比单靠情绪词汇更接近机构研报的工作方式：
