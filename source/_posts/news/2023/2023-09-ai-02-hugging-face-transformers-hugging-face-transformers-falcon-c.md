---
title: "[行业观察] Hugging Face Transformers Falcon, Code Llama, ViTDet, DINO v2, VITS：从热闹标题回到可验证事实"
date: 2023-09-06 10:00:00
updated: 2023-09-06 10:00:00
description: "人工智能｜Hugging Face Transformers Falcon, Code Llama, ViTDet, DINO v2, VITS。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2023/ai-hugging-face-transformers-hugging-face-transformers-falcon-c/
categories:
  - 资讯
tags:
  - 行业资讯
...
Hugging Face Transformers 在 **2023-09-06** 留下可核对记录：**Hugging Face Transformers Falcon, Code Llama, ViTDet, DINO v2, VITS**。AI 圈从不停，但真正值得停下来看一眼的节点通常会改写工具、成本和想象力。
...
| 一手入口 | [阅读原文](https://github.com/huggingface/transformers/releases/tag/v4.33.1) |
| 收录月 | 2023-09 |
...
- **公开信息：** Falcon port 24523 by @Rocketknight1
...
- **公开信息：** Falcon: Add RoPE scaling by @gante in 25878
...
- **公开信息：** Add proper Falcon docs and conversion script by @Rocketknight1 in 25954
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
...
- **公开信息：** Put Falcon back by @LysandreJik in 25960
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** [Falcon] Remove SDPA for falcon to support earlier versions of PyTorch (< 2.0) by @younesbelkada in 25947
...
- **公开信息：** [CodeLlama] Add support for CodeLlama by @ArthurZucker in 25740
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **我想追问的：** 失败或误解的代价由谁承担：用户、平台，还是机构？
- **可执行翻译：** 若相关，就开一个 30 分钟验证任务；若不相关，也明确写“暂不跟”。
...
- **公开信息：** [CodeLlama] Fix CI by @ArthurZucker in 25890
...
- **公开信息：** Add ViTDet by @NielsRogge in 25524
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
对「Hugging Face Transformers Falcon, Code Llama, ViTDet, DINO v2, VITS」做专业诊断时，先定位它主要改写的是结构、激励，还是约束。三者抓错，后续讨论会全程错位。

请用下面的分析骨架阅读「Hugging Face Transformers Falcon, Code Llama, ViTDet, DINO v2, VITS」，它比单靠情绪词汇更接近机构研报的工作方式：
