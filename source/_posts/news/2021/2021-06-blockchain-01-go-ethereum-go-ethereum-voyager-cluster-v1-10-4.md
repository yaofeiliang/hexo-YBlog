---
title: "[行业观察] go-ethereum Voyager Cluster (v1.10.4)：先看规则，再看热闹"
date: 2021-06-17 10:00:00
updated: 2021-06-17 10:00:00
description: "区块链｜go-ethereum Voyager Cluster (v1.10.4)。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2021/blockchain-go-ethereum-go-ethereum-voyager-cluster-v1-10-4/
categories:
  - 资讯
tags:
  - 行业资讯
...
go-ethereum Voyager Cluster (v1.10.4) 这类节点，适合当成“制度/协议变更”，而不是当成心情天气。

go-ethereum 在 **2021-06-17** 留下可核对记录：**go-ethereum Voyager Cluster (v1.10.4)**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 一手入口 | [阅读原文](https://github.com/ethereum/go-ethereum/releases/tag/v1.10.4) |
| 收录月 | 2021-06 |
...
- **公开信息：** [EIP-1559]: Fee market change for ETH 1.0 chain (22837, 22888, 22970)
...
- **公开信息：** [EIP-3198]: BASEFEE opcode (included in EIP-1559 changes)
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** [EIP-3529]: Reduction in refunds (22733)
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** [EIP-3541]: Reject new contracts starting with the 0xEF byte (22809)
...
- **公开信息：** [EIP-3554]: Difficulty Bomb Delay to December 1st 2021 (22840, 22870)
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
- **公开信息：** This release contains testnet activation block numbers for the London hard fork. The numbers are: Ropsten - 10499401, Goerli - 5062605, Rinkeby - 8897988. (23041)
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
- **公开信息：** This release does not contain automatic activation of the fork on mainnet. A release containing the mainnet activation block will be provided later, following the ACD decision.
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
- **公开信息：** As with all previous fork-related releases, we have added an override flag which can be used to set the activation block. This flag is temporary and will be removed some time after the fork has successfully activated on mainnet. (22822, 22972)
...
请用下面的分析骨架阅读「go-ethereum Voyager Cluster (v1.10.4)」，它比单靠情绪词汇更接近机构研报的工作方式：
