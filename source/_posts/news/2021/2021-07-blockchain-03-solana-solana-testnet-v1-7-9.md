---
title: "[行业观察] Solana Testnet - v1.7.9：升级、托管与退出路径"
date: 2021-07-31 10:00:00
updated: 2021-07-31 10:00:00
description: "区块链｜Solana Testnet - v1.7.9。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2021/blockchain-solana-solana-testnet-v1-7-9/
categories:
  - 资讯
tags:
  - 行业资讯
...
Solana Testnet - v1.7.9 这类节点，适合当成“制度/协议变更”，而不是当成心情天气。

Solana 在 **2021-07-31** 留下可核对记录：**Solana Testnet - v1.7.9**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 一手入口 | [阅读原文](https://github.com/solana-labs/solana/releases/tag/v1.7.9) |
| 收录月 | 2021-07 |
...
- **公开信息：** Fixes argument serialization for RpcClient::getblockproduction() (thanks to @brson)
...
- **公开信息：** Improves M1 silicon MacOS compatibility (thanks to @asubiotto)
...
- **公开信息：** Improves RocksDB column compatibility with master branch
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** Parallelizes gossip CRDS filtering
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** Adds shell-completion generation for solana CLI util (thanks to @theonekeyg)
...
请用下面的分析骨架阅读「Solana Testnet - v1.7.9」，它比单靠情绪词汇更接近机构研报的工作方式：
