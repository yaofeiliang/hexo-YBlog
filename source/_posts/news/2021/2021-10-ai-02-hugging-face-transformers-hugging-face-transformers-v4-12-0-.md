---
title: "[行业观察] Hugging Face Transformers v4.12.0: TrOCR, SEW & SEW-D, Unispeech & Unispeech-SAT, BARTPho：从热闹标题回到可验证事实"
date: 2021-10-28 10:00:00
updated: 2021-10-28 10:00:00
description: "人工智能｜Hugging Face Transformers v4.12.0: TrOCR, SEW & SEW-D, Unispeech & Unispeech-SAT, BARTPho。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2021/ai-hugging-face-transformers-hugging-face-transformers-v4-12-0-/
categories:
  - 资讯
tags:
  - 行业资讯
...
2021-10-28 的「Hugging Face Transformers v4.12.0: TrOCR, SEW & SEW-D, Unispeech & Unispeech-SAT, BARTPho」值得停一下。AI 圈从不缺口号，缺的是把边界讲清楚。

Hugging Face Transformers 在 **2021-10-28** 留下可核对记录：**Hugging Face Transformers v4.12.0: TrOCR, SEW & SEW-D, Unispeech & Unispeech-SAT, BARTPho**。AI 圈从不停，但真正值得停下来看一眼的节点通常会改写工具、成本和想象力。
...
| 一手入口 | [阅读原文](https://github.com/huggingface/transformers/releases/tag/v4.12.0) |
| 收录月 | 2021-10 |
...
- **公开信息：** Add TrOCR + VisionEncoderDecoderModel by @NielsRogge in https://github.com/huggingface/transformers/pull/13874
...
- **公开信息：** Add the SEW and SEW-D speech models by @anton-l in https://github.com/huggingface/transformers/pull/13962
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
...
- **公开信息：** Add SEW CTC models by @anton-l in https://github.com/huggingface/transformers/pull/14158
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
...
- **公开信息：** Keras callback to push to hub each epoch, or after N steps by @Rocketknight1 in https://github.com/huggingface/transformers/pull/13773
...
- **公开信息：** Add TFEncoderDecoderModel + Add cross-attention to some TF models by @ydshieh in https://github.com/huggingface/transformers/pull/13222
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
- **公开信息：** Fix EncoderDecoderModel classes to be more like BART and T5 by @NielsRogge in https://github.com/huggingface/transformers/pull/14139
...
- **公开信息：** Add DistilHuBERT by @anton-l in https://github.com/huggingface/transformers/pull/14174
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
- **公开信息：** [Speech Examples] Add pytorch speech pretraining by @patrickvonplaten in https://github.com/huggingface/transformers/pull/13877
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
请用下面的分析骨架阅读「Hugging Face Transformers v4.12.0: TrOCR, SEW & SEW-D, Unispeech & Unispeech-SAT, BARTPho」，它比单靠情绪词汇更接近机构研报的工作方式：
