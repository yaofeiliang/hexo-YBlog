---
title: "[行业观察] Solana Mainnet - v1.5.19：升级、托管与退出路径"
date: 2021-04-20 10:00:00
updated: 2021-04-20 10:00:00
description: "区块链｜Solana Mainnet - v1.5.19。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2021/blockchain-solana-solana-mainnet-v1-5-19/
categories:
  - 资讯
tags:
  - 行业资讯
...
Solana Mainnet - v1.5.19 这类节点，适合当成“制度/协议变更”，而不是当成心情天气。

Solana 在 **2021-04-20** 留下可核对记录：**Solana Mainnet - v1.5.19**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 一手入口 | [阅读原文](https://github.com/solana-labs/solana/releases/tag/v1.5.19) |
| 收录月 | 2021-04 |
...
- **公开信息：** new getInflationReward endpoint
...
- **公开信息：** unprocessed transactions are now filtered out of logs
...
- **公开信息：** correct transaction parsing for uninitialized system/nonce accounts
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** PubSub now correctly uses finalized as the default commitment level
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** solana stake-account and solana vote-account are now wicked fast; use the --with-rewards and --num-rewards-epochs parameters to control amount of reward data
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
- **公开信息：** New solana inflation rewards command allows inspecting rewards for multiple accounts during a particular epoch
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
- **公开信息：** Limit solana stake-history output by default
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 失败或误解的代价由谁承担：用户、平台，还是机构？
- **可执行翻译：** 若相关，就开一个 30 分钟验证任务；若不相关，也明确写“暂不跟”。
...
- **公开信息：** solana airdrop now uses JSON RPC instead to receive the airdrop transaction
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
请用下面的分析骨架阅读「Solana Mainnet - v1.5.19」，它比单靠情绪词汇更接近机构研报的工作方式：
