---
title: "[行业观察] go-ethereum Pangaea Expanse (v1.10.0)：把链上黑话翻译成人话"
date: 2021-03-03 10:00:00
updated: 2021-03-03 10:00:00
description: "区块链｜go-ethereum Pangaea Expanse (v1.10.0)。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2021/blockchain-go-ethereum-go-ethereum-pangaea-expanse-v1-10-0/
categories:
  - 资讯
tags:
  - 行业资讯
...
go-ethereum Pangaea Expanse (v1.10.0) 这类节点，适合当成“制度/协议变更”，而不是当成心情天气。

go-ethereum 在 **2021-03-03** 留下可核对记录：**go-ethereum Pangaea Expanse (v1.10.0)**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 一手入口 | [阅读原文](https://github.com/ethereum/go-ethereum/releases/tag/v1.10.0) |
| 收录月 | 2021-03 |
...
- **公开信息：** EIP-2929: gas cost increases for state access opcodes
...
- **公开信息：** EIP-2315: simple subroutines for the EVM
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** EIP-2565: modexp precompile gas cost changes (22213)
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** EIP-2718: typed transaction envelope (21502)
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** EIP-2930: access list transactions (21502)
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
- **公开信息：** We have fully removed many deprecated command-line flags. (22263)
...
- **公开信息：** Geth will now terminate gracefully when the disk is almost full, to avoid database corruption. (22103)
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
- **公开信息：** Geth now tracks unclean shutdowns and warns you about them when starting back up. (21893)
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
请用下面的分析骨架阅读「go-ethereum Pangaea Expanse (v1.10.0)」，它比单靠情绪词汇更接近机构研报的工作方式：
