---
title: "[行业观察] Solana Mainnet - v1.4.25：比行情截图更值得读的部分"
date: 2021-01-29 10:00:00
updated: 2021-01-29 10:00:00
description: "区块链｜Solana Mainnet - v1.4.25。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2021/blockchain-solana-solana-mainnet-v1-4-25/
categories:
  - 资讯
tags:
  - 行业资讯
...
Solana 在 **2021-01-29** 留下可核对记录：**Solana Mainnet - v1.4.25**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 一手入口 | [阅读原文](https://github.com/solana-labs/solana/releases/tag/v1.4.25) |
| 收录月 | 2021-01 |
...
- **公开信息：** Numerous solana-cli improvements, including promotion of --commitment to a global arg, new solana rent subcommand, and much more reliable program deploys
...
- **公开信息：** Add generalized voting process to enable full inflation
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
...
- **公开信息：** RPC: Add block time to getConfirmedTransaction and getConfirmedSignaturesForAddress2, and return SPL token deltas with transaction metadata
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
...
- **公开信息：** RPC: Return custom error for block skipped/not-found in long-term storage
...
- **公开信息：** Improved transaction sanitization in solana-cli and RPC service
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
- **公开信息：** Tighten buffer authority handling in the BPF upgradeable loader
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
请用下面的分析骨架阅读「Solana Mainnet - v1.4.25」，它比单靠情绪词汇更接近机构研报的工作方式：
