---
title: "[行业观察] Solana Mainnet - v1.5.8：升级、托管与退出路径"
date: 2021-02-17 10:00:00
updated: 2021-02-17 10:00:00
description: "区块链｜Solana Mainnet - v1.5.8。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2021/blockchain-solana-solana-mainnet-v1-5-8/
categories:
  - 资讯
tags:
  - 行业资讯
...
source_url: https://github.com/solana-labs/solana/releases/tag/v1.5.8
source_published: 2021-02-17
...
Solana 在 **2021-02-17** 留下可核对记录：**Solana Mainnet - v1.5.8**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 时间 | 2021-02-17 |
| 主体 | Solana |
| 事件 | Solana Mainnet - v1.5.8 |
| 一手入口 | [阅读原文](https://github.com/solana-labs/solana/releases/tag/v1.5.8) |
| 收录月 | 2021-02 |
...
- **公开信息：** Sanitize Hash base58 input
...
- **公开信息：** Log if unsanitary transactions are read from Blockstore
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** Check that prune-messages have the same inner and outer pubkeys
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** Bump solana-rbpf to 0.2.5
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** Implement more-configurable rocksdb compaction
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
- **公开信息：** Clean up & check stake program
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
请用下面的分析骨架阅读「Solana Mainnet - v1.5.8」，它比单靠情绪词汇更接近机构研报的工作方式：
...
- 原始日期：2021-02-17
- 收录月份：2021-02
- 一手链接：<https://github.com/solana-labs/solana/releases/tag/v1.5.8>
