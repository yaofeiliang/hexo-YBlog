---
title: "[行业观察] Hugging Face Transformers v4.42.0: Gemma 2, RTDETR, InstructBLIP, LLAVa Next, New Model Adder：从热闹标题回到可验证事实"
date: 2024-06-27 10:00:00
updated: 2024-06-27 10:00:00
description: "人工智能｜Hugging Face Transformers v4.42.0: Gemma 2, RTDETR, InstructBLIP, LLAVa Next, New Model Adder。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2024/ai-hugging-face-transformers-hugging-face-transformers-v4-42-0-/
categories:
  - 资讯
tags:
  - 行业资讯
...
Hugging Face Transformers 在 **2024-06-27** 留下可核对记录：**Hugging Face Transformers v4.42.0: Gemma 2, RTDETR, InstructBLIP, LLAVa Next, New Model Adder**。AI 圈从不停，但真正值得停下来看一眼的节点通常会改写工具、成本和想象力。
...
| 一手入口 | [阅读原文](https://github.com/huggingface/transformers/releases/tag/v4.42.0) |
| 收录月 | 2024-06 |
...
- **公开信息：** Add gemma 2 by @ArthurZucker in 31659
...
- **公开信息：** New model support RTDETR by @SangbumChoi in 29077
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
...
- **公开信息：** Add video modality for InstrucBLIP by @zucchini-nlp in 30182
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
...
- **公开信息：** Add LLaVa NeXT Video by @zucchini-nlp in 31252
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
...
- **公开信息：** Diff converter v2 by @ArthurZucker in 30868
...
- **公开信息：** Chat Template support for function calling and RAG by @Rocketknight1 in 30621
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **我想追问的：** 失败或误解的代价由谁承担：用户、平台，还是机构？
- **可执行翻译：** 若相关，就开一个 30 分钟验证任务；若不相关，也明确写“暂不跟”。
...
- **公开信息：** Add Qwen2 GGUF loading support by @Isotr0py in 31175
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
- **公开信息：** GGUF: Fix llama 3 GGUF by @younesbelkada in 31358
...
对「Hugging Face Transformers v4.42.0: Gemma 2, RTDETR, InstructBLIP, LLAVa Next, New Model Adder」做专业诊断时，先定位它主要改写的是结构、激励，还是约束。三者抓错，后续讨论会全程错位。

请用下面的分析骨架阅读「Hugging Face Transformers v4.42.0: Gemma 2, RTDETR, InstructBLIP, LLAVa Next, New Model Adder」，它比单靠情绪词汇更接近机构研报的工作方式：
