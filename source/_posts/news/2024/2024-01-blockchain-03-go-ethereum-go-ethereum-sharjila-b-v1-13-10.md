---
title: "[行业观察] go-ethereum Sharjila B (v1.13.10)：比行情截图更值得读的部分"
date: 2024-01-11 10:00:00
updated: 2024-01-11 10:00:00
description: "区块链｜go-ethereum Sharjila B (v1.13.10)。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2024/blockchain-go-ethereum-go-ethereum-sharjila-b-v1-13-10/
categories:
  - 资讯
tags:
  - 行业资讯
...
go-ethereum 在 **2024-01-11** 留下可核对记录：**go-ethereum Sharjila B (v1.13.10)**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 一手入口 | [阅读原文](https://github.com/ethereum/go-ethereum/releases/tag/v1.13.10) |
| 收录月 | 2024-01 |
...
- **公开信息：** The 'simulated backend' in package accounts/abi/backends was rewritten. The improved version is available from the new package ethclient/simulated. A backwards-compatibility wrapper remains in the old location. (28202)
...
- **公开信息：** Fix ABI-encoding of negative big.Int in topics (28764)
...
- **公开信息：** In JSON logging output, the "error" level is now correctly emitted as "error". (28774, 28780)
...
- **公开信息：** Fixed an issue with configuration of stdlib package log for consumers of the geth library (28747)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
...
- **公开信息：** geth removedb can now be run non-interactively (28725)
...
- **公开信息：** We're building a package for ubuntu 23.10: mantic minotaur now (28728)
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
- **公开信息：** Add currentExcessBlobGas to the state tests for better coverage of state tests (28735)
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
- **公开信息：** Fixed an issue in t8n regarding blob gas usage (28735)
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
请用下面的分析骨架阅读「go-ethereum Sharjila B (v1.13.10)」，它比单靠情绪词汇更接近机构研报的工作方式：
