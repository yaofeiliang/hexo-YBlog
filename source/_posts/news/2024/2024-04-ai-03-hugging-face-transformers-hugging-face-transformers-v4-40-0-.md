---
title: "[行业观察] Hugging Face Transformers v4.40.0: Llama 3, Idefics 2, Recurrent Gemma, Jamba, DBRX, OLMo, Qwen2MoE, Grounding Dino：别被演示绕晕，先看成本和边界"
date: 2024-04-18 10:00:00
updated: 2024-04-18 10:00:00
description: "人工智能｜Hugging Face Transformers v4.40.0: Llama 3, Idefics 2, Recurrent Gemma, Jamba, DBRX, OLMo, Qwen2MoE, Grounding Dino。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2024/ai-hugging-face-transformers-hugging-face-transformers-v4-40-0-/
categories:
  - 资讯
tags:
  - 行业资讯
...
Hugging Face Transformers 在 **2024-04-18** 留下可核对记录：**Hugging Face Transformers v4.40.0: Llama 3, Idefics 2, Recurrent Gemma, Jamba, DBRX, OLMo, Qwen2MoE, Grounding Dino**。AI 圈从不停，但真正值得停下来看一眼的节点通常会改写工具、成本和想象力。
...
| 一手入口 | [阅读原文](https://github.com/huggingface/transformers/releases/tag/v4.40.0) |
| 收录月 | 2024-04 |
...
- **公开信息：** Add Idefics2 by @amyeroberts in 30253
...
- **公开信息：** Add recurrent gemma by @ArthurZucker in 30143
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
...
- **公开信息：** Add jamba by @tomeras91 in 29943
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
...
- **公开信息：** Add DBRX Model by @abhi-mosaic in 29921
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
...
- **公开信息：** Add OLMo model family by @2015aroras in 29890
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
- **公开信息：** Add Qwen2MoE by @bozheng-hit in 29377
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
- **公开信息：** Adding grounding dino by @EduardoPach in 26087
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
- **公开信息：** Remove static pretrained maps from the library's internals by @LysandreJik in 29112
...
请用下面的分析骨架阅读「Hugging Face Transformers v4.40.0: Llama 3, Idefics 2, Recurrent Gemma, Jamba, DBRX, OLMo, Qwen2MoE, Grounding Dino」，它比单靠情绪词汇更接近机构研报的工作方式：
