---
title: "[行业观察] go-ethereum Edolus (v1.13.12)：比行情截图更值得读的部分"
date: 2024-02-09 10:00:00
updated: 2024-02-09 10:00:00
description: "区块链｜go-ethereum Edolus (v1.13.12)。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2024/blockchain-go-ethereum-go-ethereum-edolus-v1-13-12/
categories:
  - 资讯
tags:
  - 行业资讯
...
source_url: https://github.com/ethereum/go-ethereum/releases/tag/v1.13.12
source_published: 2024-02-09
...
go-ethereum 在 **2024-02-09** 留下可核对记录：**go-ethereum Edolus (v1.13.12)**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 时间 | 2024-02-09 |
| 主体 | go-ethereum |
| 事件 | go-ethereum Edolus (v1.13.12) |
| 一手入口 | [阅读原文](https://github.com/ethereum/go-ethereum/releases/tag/v1.13.12) |
| 收录月 | 2024-02 |
...
- **公开信息：** EIP-1153: Transient storage opcodes
...
- **公开信息：** EIP-4788: Beacon block root in the EVM
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** EIP-4844: Shard Blob Transactions
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** EIP-5656: MCOPY - Memory copying instruction
...
- **公开信息：** EIP-6780: SELFDESTRUCT only in same transaction
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 失败或误解的代价由谁承担：用户、平台，还是机构？
- **可执行翻译：** 若相关，就开一个 30 分钟验证任务；若不相关，也明确写“暂不跟”。
...
- **公开信息：** EIP-7516: BLOBBASEFEE opcode
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
- **公开信息：** Geth Cancun InfluxDB Dashboard
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
- **公开信息：** Geth Cancun Prometheus Dashboard
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
请用下面的分析骨架阅读「go-ethereum Edolus (v1.13.12)」，它比单靠情绪词汇更接近机构研报的工作方式：
...
- 原始日期：2024-02-09
- 收录月份：2024-02
- 一手链接：<https://github.com/ethereum/go-ethereum/releases/tag/v1.13.12>
