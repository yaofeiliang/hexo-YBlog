---
title: "[行业观察] go-ethereum Paragade (v1.9.20)：先看规则，再看热闹"
date: 2020-08-25 10:00:00
updated: 2020-08-25 10:00:00
description: "区块链｜go-ethereum Paragade (v1.9.20)。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2020/blockchain-go-ethereum-go-ethereum-paragade-v1-9-20/
categories:
  - 资讯
tags:
  - 行业资讯
...
go-ethereum 在 **2020-08-25** 留下可核对记录：**go-ethereum Paragade (v1.9.20)**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 一手入口 | [阅读原文](https://github.com/ethereum/go-ethereum/releases/tag/v1.9.20) |
| 收录月 | 2020-08 |
...
- **公开信息：** You can now query the Chain ID using GraphQL (21451)
...
- **公开信息：** The evm command can now appends the TX hash to state transition output files (21406)
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** Blockchain rewinding and chain repair, i.e. using the SetHead operation, now behave
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** Discovery DHT bootstrapping now works properly in very small private networks. (21396)
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** Contract code is now stored separately from state tree data in LevelDB. This change will
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
- **公开信息：** Two additional metrics counters for blockchain reorgs have been added. (21420)
...
- **公开信息：** Metrics collection is now lock-free, causing less lock contention among peer connections (21446, 21470)
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
- **公开信息：** The GetNodeData operation in the eth peer-to-peer protocol now uses the fast-sync bloom filter
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 失败或误解的代价由谁承担：用户、平台，还是机构？
- **可执行翻译：** 若相关，就开一个 30 分钟验证任务；若不相关，也明确写“暂不跟”。
...
对「go-ethereum Paragade (v1.9.20)」做专业诊断时，先定位它主要改写的是结构、激励，还是约束。三者抓错，后续讨论会全程错位。

请用下面的分析骨架阅读「go-ethereum Paragade (v1.9.20)」，它比单靠情绪词汇更接近机构研报的工作方式：
