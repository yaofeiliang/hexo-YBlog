---
title: "[行业观察] Hugging Face Transformers Reformer, ElectraForSequenceClassification, ONNX conversion script：从热闹标题回到可验证事实"
date: 2020-05-22 10:00:00
updated: 2020-05-22 10:00:00
description: "人工智能｜Hugging Face Transformers Reformer, ElectraForSequenceClassification, ONNX conversion script。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2020/ai-hugging-face-transformers-hugging-face-transformers-reformer/
categories:
  - 资讯
tags:
  - 行业资讯
...
Hugging Face Transformers 在 **2020-05-22** 留下可核对记录：**Hugging Face Transformers Reformer, ElectraForSequenceClassification, ONNX conversion script**。AI 圈从不停，但真正值得停下来看一眼的节点通常会改写工具、成本和想象力。
...
| 一手入口 | [阅读原文](https://github.com/huggingface/transformers/releases/tag/v2.10.0) |
| 收录月 | 2020-05 |
...
- **公开信息：** Added a new model "Reformer": https://arxiv.org/abs/2001.04451 to the library. Original trax code: https://github.com/google/trax/tree/master/trax/models/reformer was translated to PyTorch.
...
- **公开信息：** Reformer uses chunked attention and reversible layers to model sequences as long as 500,000 tokens.
...
- **公开信息：** Reformer is currently available as a casual language model and will soon also be available as encoder only ("Bert"-like) model.
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** Two pretrained weights are uploaded: https://huggingface.co/models?search=google%2Freformer
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** https://huggingface.co/google/reformer-enwik8 is the first char lm in the library
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
- **公开信息：** The ElectraForSequenceClassification was added by @liuzzi
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **我想追问的：** 失败或误解的代价由谁承担：用户、平台，还是机构？
- **可执行翻译：** 若相关，就开一个 30 分钟验证任务；若不相关，也明确写“暂不跟”。
...
- **公开信息：** Model saving, as well as optimizer and scheduler saving mid-training were hanging
...
- **公开信息：** Fixed the optimizer weight updates
...
对「Hugging Face Transformers Reformer, ElectraForSequenceClassification, ONNX conversion script」做专业诊断时，先定位它主要改写的是结构、激励，还是约束。三者抓错，后续讨论会全程错位。

请用下面的分析骨架阅读「Hugging Face Transformers Reformer, ElectraForSequenceClassification, ONNX conversion script」，它比单靠情绪词汇更接近机构研报的工作方式：
