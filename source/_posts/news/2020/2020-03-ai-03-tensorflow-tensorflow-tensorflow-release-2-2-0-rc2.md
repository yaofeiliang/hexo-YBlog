---
title: "[行业观察] TensorFlow TensorFlow Release 2.2.0-rc2：给非研究员的实用导读"
date: 2020-03-27 10:00:00
updated: 2020-03-27 10:00:00
description: "人工智能｜TensorFlow TensorFlow Release 2.2.0-rc2。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2020/ai-tensorflow-tensorflow-tensorflow-release-2-2-0-rc2/
categories:
  - 资讯
tags:
  - 行业资讯
...
TensorFlow 在 **2020-03-27** 留下可核对记录：**TensorFlow TensorFlow Release 2.2.0-rc2**。AI 圈从不停，但真正值得停下来看一眼的节点通常会改写工具、成本和想象力。
...
| 一手入口 | [阅读原文](https://github.com/tensorflow/tensorflow/releases/tag/v2.2.0-rc2) |
| 收录月 | 2020-03 |
...
- **公开信息：** Replaced the scalar type for string tensors from std::string to tensorflow::tstring which is now ABI stable.
...
- **公开信息：** A new Profiler for TF 2 for CPU/GPU/TPU. It offers both device and host performance analysis, including input pipeline and TF Ops. Optimization advisory is provided whenever possible. Please see this tutorial and guide for usage guidelines.
...
- **公开信息：** Export C++ functions to Python using pybind11 as opposed to SWIG as a part of our deprecation of swig efforts.
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** Support added for global sync BatchNormalization by using the newly added tf.keras.layers.experimental.SyncBatchNormalization layer. This layer will sync BatchNormalization statistics every step across all replicas taking part in sync training.
...
- **公开信息：** Performance improvements for GPU multi-worker distributed training using tf.distribute.experimental.MultiWorkerMirroredStrategy
...
- **公开信息：** Update NVIDIA NCCL to 2.5.7-1 for better performance and performance tuning. Please see nccl developer guide for more information on this.
- **阅读提醒：** 性能信号：必须落到你们的基准场景（P95 延迟、构建时长、包体、内存）复测，避免被形容词绑架。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
- **公开信息：** Support gradient allreduce in float16. See this example usage.
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
请用下面的分析骨架阅读「TensorFlow TensorFlow Release 2.2.0-rc2」，它比单靠情绪词汇更接近机构研报的工作方式：
