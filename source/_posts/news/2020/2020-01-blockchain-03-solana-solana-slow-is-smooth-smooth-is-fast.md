---
title: "[行业观察] Solana Slow is Smooth, Smooth is Fast：先看规则，再看热闹"
date: 2020-01-28 10:00:00
updated: 2020-01-28 10:00:00
description: "区块链｜Solana Slow is Smooth, Smooth is Fast。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2020/blockchain-solana-solana-slow-is-smooth-smooth-is-fast/
categories:
  - 资讯
tags:
  - 行业资讯
...
Solana Slow is Smooth, Smooth is Fast 这类节点，适合当成“制度/协议变更”，而不是当成心情天气。

Solana 在 **2020-01-28** 留下可核对记录：**Solana Slow is Smooth, Smooth is Fast**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 一手入口 | [阅读原文](https://github.com/solana-labs/solana/releases/tag/v0.23.0) |
| 收录月 | 2020-01 |
...
- **公开信息：** Simplified voting and staking rewards redemption
...
- **公开信息：** Solana command-line improvements:
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** New stakes subcommand to view all cluster stakes
...
- **公开信息：** New create-address-with-seed to generate derived account addresses
- **阅读提醒：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
...
- **公开信息：** Renamed various subcommands for consistency
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
- **公开信息：** Documentation updates for inflation
...
- **公开信息：** Stability and security fixes
...
请用下面的分析骨架阅读「Solana Slow is Smooth, Smooth is Fast」，它比单靠情绪词汇更接近机构研报的工作方式：
