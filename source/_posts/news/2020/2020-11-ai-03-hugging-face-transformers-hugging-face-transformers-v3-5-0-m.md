---
title: "[行业观察] Hugging Face Transformers v3.5.0: Model versioning, TensorFlow encoder-decoder models, new scripts, refactor of the `generate` method：从热闹标题回到可验证事实"
date: 2020-11-10 10:00:00
updated: 2020-11-10 10:00:00
description: "人工智能｜Hugging Face Transformers v3.5.0: Model versioning, TensorFlow encoder-decoder models, new scripts, refactor of the `generate` method。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2020/ai-hugging-face-transformers-hugging-face-transformers-v3-5-0-m/
categories:
  - 资讯
tags:
  - 行业资讯
...
Hugging Face Transformers 在 **2020-11-10** 留下可核对记录：**Hugging Face Transformers v3.5.0: Model versioning, TensorFlow encoder-decoder models, new scripts, refactor of the `generate` method**。AI 圈从不停，但真正值得停下来看一眼的节点通常会改写工具、成本和想象力。
...
| 一手入口 | [阅读原文](https://github.com/huggingface/transformers/releases/tag/v3.5.0) |
| 收录月 | 2020-11 |
...
- **公开信息：** built-in versioning (I mean… it’s git. It’s pretty much what you use for versioning. Versioning in S3 has a ton a limitations)
...
- **公开信息：** access control (will unlock private models, private datasets, etc)
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** scalability (our usage of S3 to maintain lists of models was starting to bottleneck)
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** see commit histories and diffs of changes made to any text file, like config.json:
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** changes made by the HuggingFace team will be way clearer – we can perform updates to the models to ensure they work well with the library(ies) (you'll be able to opt out from those changes)
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
- **公开信息：** Large binary files are stored using https://git-lfs.github.com/ which is pretty standard now, and interoperable out of the box with git
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 失败或误解的代价由谁承担：用户、平台，还是机构？
- **可执行翻译：** 若相关，就开一个 30 分钟验证任务；若不相关，也明确写“暂不跟”。
...
- **公开信息：** Ability to update your text files, like your README.md model card, directly on the website!
...
对「Hugging Face Transformers v3.5.0: Model versioning, TensorFlow encoder-decoder models, new scripts, refactor of the `generate` method」做专业诊断时，先定位它主要改写的是结构、激励，还是约束。三者抓错，后续讨论会全程错位。

请用下面的分析骨架阅读「Hugging Face Transformers v3.5.0: Model versioning, TensorFlow encoder-decoder models, new scripts, refactor of the `generate` method」，它比单靠情绪词汇更接近机构研报的工作方式：
