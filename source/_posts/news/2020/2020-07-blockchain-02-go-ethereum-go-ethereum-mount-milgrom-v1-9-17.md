---
title: "[行业观察] go-ethereum Mount Milgrom (v1.9.17)：升级、托管与退出路径"
date: 2020-07-20 10:00:00
updated: 2020-07-20 10:00:00
description: "区块链｜go-ethereum Mount Milgrom (v1.9.17)。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2020/blockchain-go-ethereum-go-ethereum-mount-milgrom-v1-9-17/
categories:
  - 资讯
tags:
  - 行业资讯
...
go-ethereum Mount Milgrom (v1.9.17) 这类节点，适合当成“制度/协议变更”，而不是当成心情天气。

go-ethereum 在 **2020-07-20** 留下可核对记录：**go-ethereum Mount Milgrom (v1.9.17)**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 一手入口 | [阅读原文](https://github.com/ethereum/go-ethereum/releases/tag/v1.9.17) |
| 收录月 | 2020-07 |
...
- **公开信息：** Enable historical garbage collection for light clients (19570).
...
- **公开信息：** Apply --rpc.txfeecap to a few missed endpoints (21231).
...
- **公开信息：** Drastically reduce allocations in the transaction pool (21328).
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** Drastically reduce allocations on certain EVM opcodes (21222).
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** Raise the default gas limit in --dev mode to 12 million (21323).
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
- **公开信息：** Fix ethstats reconnect issue and fix constant Görli drops (21347).
...
- **公开信息：** Fix gas estimation if balance / price overflown uint64 (21346).
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 失败或误解的代价由谁承担：用户、平台，还是机构？
- **可执行翻译：** 若相关，就开一个 30 分钟验证任务；若不相关，也明确写“暂不跟”。
...
请用下面的分析骨架阅读「go-ethereum Mount Milgrom (v1.9.17)」，它比单靠情绪词汇更接近机构研报的工作方式：
