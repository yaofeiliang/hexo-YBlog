---
title: "[行业观察] Hugging Face Transformers ProphetNet, Blenderbot, SqueezeBERT, DeBERTa：从热闹标题回到可验证事实"
date: 2020-10-20 10:00:00
updated: 2020-10-20 10:00:00
description: "人工智能｜Hugging Face Transformers ProphetNet, Blenderbot, SqueezeBERT, DeBERTa。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2020/ai-hugging-face-transformers-hugging-face-transformers-prophetn/
categories:
  - 资讯
tags:
  - 行业资讯
...
2020-10-20 的「Hugging Face Transformers ProphetNet, Blenderbot, SqueezeBERT, DeBERTa」值得停一下。AI 圈从不缺口号，缺的是把边界讲清楚。

Hugging Face Transformers 在 **2020-10-20** 留下可核对记录：**Hugging Face Transformers ProphetNet, Blenderbot, SqueezeBERT, DeBERTa**。AI 圈从不停，但真正值得停下来看一眼的节点通常会改写工具、成本和想象力。
...
| 一手入口 | [阅读原文](https://github.com/huggingface/transformers/releases/tag/v3.4.0) |
| 收录月 | 2020-10 |
...
- **公开信息：** microsoft/xprophetnet-large-wiki100-cased-xglue-ntg
...
- **公开信息：** microsoft/prophetnet-large-uncased
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** microsoft/prophetnet-large-uncased-cnndm
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** microsoft/xprophetnet-large-wiki100-cased
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** microsoft/xprophetnet-large-wiki100-cased-xglue-qg
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
- **公开信息：** ProphetNet 7157 (@qiweizhen, @patrickvonplaten)
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 失败或误解的代价由谁承担：用户、平台，还是机构？
- **可执行翻译：** 若相关，就开一个 30 分钟验证任务；若不相关，也明确写“暂不跟”。
...
请用下面的分析骨架阅读「Hugging Face Transformers ProphetNet, Blenderbot, SqueezeBERT, DeBERTa」，它比单靠情绪词汇更接近机构研报的工作方式：
