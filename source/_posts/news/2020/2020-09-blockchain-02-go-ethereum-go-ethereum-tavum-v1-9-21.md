---
title: "[行业观察] go-ethereum Tavum (v1.9.21)：比行情截图更值得读的部分"
date: 2020-09-09 10:00:00
updated: 2020-09-09 10:00:00
description: "区块链｜go-ethereum Tavum (v1.9.21)。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2020/blockchain-go-ethereum-go-ethereum-tavum-v1-9-21/
categories:
  - 资讯
tags:
  - 行业资讯
...
go-ethereum Tavum (v1.9.21) 这类节点，适合当成“制度/协议变更”，而不是当成心情天气。

go-ethereum 在 **2020-09-09** 留下可核对记录：**go-ethereum Tavum (v1.9.21)**。链上世界热闹时像夜市，冷静时像工地——真正值钱的，常常是升级、清算和规则变化。
...
| 一手入口 | [阅读原文](https://github.com/ethereum/go-ethereum/releases/tag/v1.9.21) |
| 收录月 | 2020-09 |
...
- **公开信息：** Remove Whisper as promised a couple months ago (21487, 21526, 21527)!
...
- **公开信息：** Minor user experience polishes around legacy Ledger derivation paths (21517).
...
- **公开信息：** Implement arbitrary call tracing via debugtraceCall on top of arbitrary blocks (21338).
...
- **公开信息：** Expose internal transaction revertals and revert reason in the calltracer tracer (21387).
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** Cap the number of in-memory trie nodes during fast sync, fix crasher memory leak (21491).
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 失败或误解的代价由谁承担：用户、平台，还是机构？
- **可执行翻译：** 若相关，就开一个 30 分钟验证任务；若不相关，也明确写“暂不跟”。
...
- **公开信息：** Prepare the trie syncer for path-based operation to support the upcoming snap sync (21504).
...
- **公开信息：** Limit the cached data in the downloader more aggressively to avoid memory fluctuations (21366).
...
- **公开信息：** Fix the simulated chain to not allow changing block timestamps if transactions were included (21334).
- **阅读提醒：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
请用下面的分析骨架阅读「go-ethereum Tavum (v1.9.21)」，它比单靠情绪词汇更接近机构研报的工作方式：
