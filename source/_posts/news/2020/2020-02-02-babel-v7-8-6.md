---
title: "[技术观察] Babel v7.8.6：别只看版本号，先看你会不会被影响"
date: 2020-02-27 09:00:00
updated: 2020-02-27 09:00:00
description: "Babel 于 2020-02-27 发布 v7.8.6。结合官方条目、场景推演与升级检查，帮你判断要不要跟、怎么跟。"
permalink: news/2020/babel-v7-8-6/
categories:
  - 资讯
tags:
  - 技术资讯
...
source_url: https://github.com/babel/babel/releases/tag/v7.8.6
source_published: 2020-02-27
...
又一条值得记账的发布：Babel v7.8.6（2020-02-27）。标题可以热闹，决策还是得冷静。

若压缩成决策语言： Babel 的发布页是事实源头；我做的是把英文/术语密集的说明拆成可以讨论、可以排期、可以回滚的中文导读。带“官方”字样的内容来自发布页；场景和检查清单是通用工作方法，不表示某一条一定是本版本新增能力。

这不是人人都要半夜升级的警报。真正相关的是已经把 Babel 写进构建、部署或日常开发链路的人；其他人把它当行业动态就好。

一手入口在这里：<https://github.com/babel/babel/releases/tag/v7.8.6>
...
| 是否预发布 | 否（正式发布版本） |
| 仓库 | babel/babel |
| 官方发布页 | [查看原文](https://github.com/babel/babel/releases/tag/v7.8.6) |
...
- **官方原文：** babel-helper-module-transforms, babel-helper-replace-supers, babel-plugin-transform-modules-commonjs
  - **怎么读：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **官方原文：** #11109 Fix rewriteThis in helper-module-transforms for computed class elements (@sidntrivedi012)
  - **怎么读：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **官方原文：** babel-parser
  - **怎么读：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **官方原文：** #10956 Refactor await/yield production parameter tracking (@JLHwung)
  - **怎么读：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。

### :rocket: New Feature

- **官方原文：** babel-helper-create-regexp-features-plugin, babel-plugin-transform-classes, babel-plugin-transform-named-capturing-groups-regex, babel-preset-env
  - **怎么读：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **官方原文：** #11134 Mark wrapNativeSuper and wrapRegExp as pure (@nicolo-ribaudo)
  - **怎么读：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。

### :bug: Bug Fix

- **官方原文：** babel-helper-compilation-targets
  - **怎么读：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **官方原文：** #11124 fix: search for browserslist if esmodules is falsy (@fengzilong)
  - **怎么读：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **官方原文：** babel-register
  - **怎么读：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **官方原文：** #11160 fix: workaround misleading node.js ENOENT error (@JLHwung)
  - **怎么读：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **官方原文：** babel-parser
  - **怎么读：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **官方原文：** #11146 Parse declare modifier around accessibility modifiers (@JLHwung)
  - **怎么读：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **官方原文：** #11092 Fix Async Generic After Await Parsing Error (@liamfd)
  - **怎么读：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **官方原文：** babel-compat-data, babel-preset-env
  - **怎么读：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。

### :nailcare: Polish

- **官方原文：** #11136 fix @babel/traverse can't use path.remove() with noScope (@liuxingbaoyu)
  - **怎么读：** 兼容性信号：旧接口/旧配置可能失效，应进入变更影响评估（impact assessment），而不是只做冒烟。
- **官方原文：** babel-helper-create-class-features-plugin, babel-helper-replace-supers
  - **怎么读：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **官方原文：** #11068 Fix classNameTDZError in computed prototype methods with class fields (@sidntrivedi012)
  - **怎么读：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **官方原文：** babel-template
  - **怎么读：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **官方原文：** #11112 Respect preserveComments option in tempate.ast() (@dentrado)
  - **怎么读：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **官方原文：** babel-plugin-transform-for-of
  - **怎么读：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **官方原文：** #11088 Allow redeclaration of loop variable in body (@openorclose)
  - **怎么读：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **官方原文：** babel-helper-module-transforms, babel-plugin-transform-modules-commonjs
  - **怎么读：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。

### :house: Internal

- **官方原文：** #10797 fix: specify sourceFileName when generating inline sourcemaps (@JLHwung)
  - **怎么读：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **官方原文：** babel-types
  - **怎么读：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **官方原文：** #11089 Make isReferenced return false for method parameter name (@brokensandals)
  - **怎么读：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **官方原文：** babel-core, babel-generator, babel-plugin-external-helpers, babel-plugin-proposal-async-generator-functions, babel-plugin-proposal-class-properties, babel-plugin-proposal-decorators, babel-plugin-proposal-function-bind, babel-plugin-transform-async-to-generator, babel-plugin-transform-classes, babel-plugin-transform-flow-comments, babel-plugin-transform-flow-strip-types, babel-plugin-transform-function-name, babel-plugin-transform-parameters, babel-plugin-transform-react-constant-elements, babel-plugin-transform-react-jsx, babel-plugin-transform-regenerator, babel-plugin-transform-runtime, babel-plugin-transform-typescript, babel-preset-env
  - **怎么读：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **官方原文：** #11133 Skip newlines around inline #PURE annotations (@nicolo-ribaudo)
  - **怎么读：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **官方原文：** Other
  - **怎么读：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **官方原文：** #11175 replace gulp-watch by gulp.watch (@JLHwung)
  - **怎么读：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **官方原文：** #11163 chore: migrate lock threads to github actions (@JLHwung)
  - **怎么读：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。

### Committers: 12

- **官方原文：** babel-generator, babel-helper-fixtures, babel-parser, babel-traverse
  - **怎么读：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **官方原文：** #11168 Enable more eslint recommended rules (@JLHwung)
  - **怎么读：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **官方原文：** babel-helper-replace-supers
  - **怎么读：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **官方原文：** #11121 Fix typing of ReplaceSupers options (@existentialism)
  - **怎么读：** 缺陷修复信号：把条目映射到你们是否复现过同类故障；有则收益高，无则仍建议看回归面。
- **官方原文：** babel-core, babel-plugin-syntax-decorators, babel-template, babel-traverse
  - **怎么读：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **官方原文：** #11119 Add eslint-plugin-jest (@JLHwung)
  - **怎么读：** 能力扩展信号：先判断它解决的是产品瓶颈、工程效率还是可观测性，再排优先级。
- **官方原文：** babel-parser
  - **怎么读：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **官方原文：** #11096 Wrap type-only class fields in flow comments (@nicolo-ribaudo)
  - **怎么读：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
对 Babel（v7.8.6 / v7.8.6），建议按下列层面对齐阅读，而不是只扫版本号：
...
你可以直接反对我的权重分配，但请把反对写成可检验命题——那才是研究，而不是情绪。 下一回再遇到 Babel 的发布，你就不是从零开始，而是在更新一张已经存在的风险地图。
...
- 原始发布日期：2020-02-27
- 收录月份：2020-02
- 发布页：<https://github.com/babel/babel/releases/tag/v7.8.6>
