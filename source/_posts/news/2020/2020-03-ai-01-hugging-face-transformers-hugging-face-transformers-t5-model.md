---
title: "[行业观察] Hugging Face Transformers T5 Model, BART summarization example and reduced memory, translation pipeline：别被演示绕晕，先看成本和边界"
date: 2020-03-30 10:00:00
updated: 2020-03-30 10:00:00
description: "人工智能｜Hugging Face Transformers T5 Model, BART summarization example and reduced memory, translation pipeline。结合一手来源、场景推演与可执行跟进步骤，尽量读完有收获。"
permalink: news/2020/ai-hugging-face-transformers-hugging-face-transformers-t5-model/
categories:
  - 资讯
tags:
  - 行业资讯
...
Hugging Face Transformers 在 **2020-03-30** 留下可核对记录：**Hugging Face Transformers T5 Model, BART summarization example and reduced memory, translation pipeline**。AI 圈从不停，但真正值得停下来看一眼的节点通常会改写工具、成本和想象力。
...
| 一手入口 | [阅读原文](https://github.com/huggingface/transformers/releases/tag/v2.7.0) |
| 收录月 | 2020-03 |
...
- **公开信息：** model available in Hugging Face's community models
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** Remove the LM head and use the embedding matrix instead (~200MB)
...
- **公开信息：** Call encoder before expanding inputids (~1GB)
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
...
- **公开信息：** SelfAttention only returns weights if config.outputattentions (~500MB)
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 它适用于谁？有没有明确排除的对象？
- **可执行翻译：** 对照原文截取关键句（含日期），避免后续讨论变成口口相传。
...
- **公开信息：** Two separate, smaller decoder attention masks (~500MB)
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 生效时间是立刻、分阶段，还是仍在征求意见？
- **可执行翻译：** 把该点写进团队周报的“外部变量”一栏，并指定一位核对人。
...
- **公开信息：** drop columns that are exclusively padtokenid from inputids in evaluatecnn example.
- **阅读提醒：** 把它当作待核验的工程信号：回到模块、配置键与调用链，确认是否命中你们的依赖图。
- **我想追问的：** 若我的流程依赖旧假设，最小验证动作是什么？
- **可执行翻译：** 在笔记里用两句话复述：发生了什么、我是否被点名。
...
对「Hugging Face Transformers T5 Model, BART summarization example and reduced memory, translation pipeline」做专业诊断时，先定位它主要改写的是结构、激励，还是约束。三者抓错，后续讨论会全程错位。

请用下面的分析骨架阅读「Hugging Face Transformers T5 Model, BART summarization example and reduced memory, translation pipeline」，它比单靠情绪词汇更接近机构研报的工作方式：
